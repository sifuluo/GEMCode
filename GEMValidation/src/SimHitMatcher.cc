  ...
  if (hasCSCGeometry_) {
    ...
    if (gemvalidation::getByToken(cscSimHitInput_, csc_hits, event())) {
      ...
      if (runCSCSimHit_) {
        ...
        if (verboseCSC_) {
          cout << "nSimHits " << no << " nTrackIds " << track_ids.size() << " nSelectedCSCSimHits "
               << csc_hits_select.size() << endl;
          ...
          for (const auto& id : detIdsCSC(0)) {
            ...
            if (cscid.station() == 1 and (cscid.ring() == 1 or cscid.ring() == 4)) {
              cout << "cscdetid " << CSCDetId(id) << ": " << csc_simhits.size() << " " << csc_simhits_gp.phi() << " "
                   << csc_detid_to_hits_[id].size() << endl;
  ...
  if (hasGEMGeometry_) {
    ...
    if (gemvalidation::getByToken(gemSimHitInput_, gem_hits, event())) {
      ...
      for (const auto& h : *gem_hits.product()) {
        ...
        if (useGEMChamberType(gemvalidation::toGEMType(id.station(), id.ring())))
          gem_hits_select.push_back(h);
      }
      ...
      if (runGEMSimHit_) {
        ...
        if (verboseGEM_) {
          cout << "nSimHits " << no << " nTrackIds " << track_ids.size() << " nSelectedGEMSimHits "
               << (*gem_hits.product()).size() << endl;
          ...
          for (const auto& id : gem_ch_ids) {
            ...
            cout << "gemchid " << GEMDetId(id) << ": nHits " << gem_simhits.size() << " phi " << gem_simhits_gp.phi()
                 << " nCh " << gem_chamber_to_hits_[id].size() << endl;
            // const auto& strips = hitStripsInDetId(id);
            // cout<<"nStrip "<<strips.size()<<endl;
          ...
          for (const auto& id : gem_sch_ids) {
            ...
            const auto& gem_simhits_gp = simHitsMeanPosition(gem_simhits);
            cout << "gemschid " << GEMDetId(id) << ": " << nCoincidencePadsWithHits() << " | " << gem_simhits.size()
  ...
  if (hasME0Geometry_) {
    ...
    if (gemvalidation::getByToken(me0SimHitInput_, me0_hits, event())) {
      if (runME0SimHit_) {
        ...
        if (verboseME0_) {
          cout << "nSimHits " << no << " nTrackIds " << track_ids.size() << " ME0SimHits "
               << (*me0_hits.product()).size() << endl;
          ...
          for (const auto& id : me0_ch_ids) {
            ...
            cout << "me0chid " << ME0DetId(id) << ": nHits " << me0_simhits.size() << " phi " << me0_simhits_gp.phi()
                 << " nCh " << me0_chamber_to_hits_[id].size() << endl;
            // const auto& strips = hitStripsInDetId(id);
            // cout<<"nStrip "<<strips.size()<<endl;
            // cout<<"strips : "; std::copy(strips.begin(), strips.end(), ostream_iterator<int>(cout, " ")); cout<<endl;
          }
  ...
  if (hasRPCGeometry_) {
    ...
    if (gemvalidation::getByToken(rpcSimHitInput_, rpc_hits, event())) {
      ...
      for (const auto& h : *rpc_hits.product()) {
        ...
        if (useRPCChamberType(gemvalidation::toRPCType(id.region(), id.station(), id.ring())))
          rpc_hits_select.push_back(h);
      }
      ...
      if (runRPCSimHit_) {
        ...
        if (verboseRPC_) {
          cout << "nSimHits " << no << " nTrackIds " << track_ids.size() << " nSelectedRPCSimHits "
               << (*rpc_hits.product()).size() << endl;
          ...
          for (const auto& id : rpc_ch_ids) {
            ...
            cout << "RPCDetId " << RPCDetId(id) << ": nHits " << rpc_simhits.size() << " eta " << rpc_simhits_gp.eta()
                 << " phi " << rpc_simhits_gp.phi() << " nCh " << rpc_chamber_to_hits_[id].size() << endl;
            const auto& strips = hitStripsInDetId(id);
  ...
  if (hasDTGeometry_) {
    ...
    if (gemvalidation::getByToken(dtSimHitInput_, dt_hits, event())) {
      ...
      for (const auto& h : *dt_hits.product()) {
        ...
        if (useDTChamberType(gemvalidation::toDTType(id.wheel(), id.station())))
          dt_hits_select.push_back(h);
      }
      ...
      if (runDTSimHit_) {
        ...
        if (verboseDT_) {
          cout << "nSimHits " << no << " nTrackIds " << track_ids.size() << " nSelectedDTSimHits "
               << (*dt_hits.product()).size() << endl;
          ...
          for (const auto& id : dt_det_ids) {
            ...
            cout << "DTWireId " << DTWireId(id) << ": nHits " << dt_simhits.size() << " eta " << dt_simhits_gp.eta()
                 << " phi " << dt_simhits_gp.phi() << " nCh " << dt_chamber_to_hits_[id].size() << endl;
...
std::vector<unsigned int> SimHitMatcher::getIdsOfSimTrackShower(unsigned int initial_trk_id,
  ...
  if (!(simMuOnlyGEM_ || simMuOnlyCSC_ || simMuOnlyDT_ || simMuOnlyME0_ || simMuOnlyRPC_))
    return result;
...
void SimHitMatcher::matchCSCSimHitsToSimTrack(std::vector<unsigned int> track_ids,
  ...
  for (const auto& track_id : track_ids) {
    for (const auto& h : csc_hits) {
      ...
      if (simMuOnlyCSC_ && std::abs(pdgid) != 13)
        continue;
      // discard electron hits in the CSC chambers
      if (discardEleHitsCSC_ && pdgid == 11)
        continue;
      ...
      if (gemvalidation::is_csc(h.detUnitId())) {
        ...
        if (verboseCSC_)
          std::cout << " csc id " << CSCDetId(h.detUnitId()) << " x " << gp.x() << " y " << gp.y() << " z " << gp.z()
                    << std::endl;
      }
...
void SimHitMatcher::matchRPCSimHitsToSimTrack(std::vector<unsigned int> track_ids,
  ...
  for (const auto& track_id : track_ids) {
    for (const auto& h : rpc_hits) {
      if (h.trackId() != track_id)
        continue;
      int pdgid = h.particleType();
      if (simMuOnlyRPC_ && std::abs(pdgid) != 13)
        continue;
      // discard electron hits in the RPC chambers
      if (discardEleHitsRPC_ && pdgid == 11)
        continue;
...
void SimHitMatcher::matchGEMSimHitsToSimTrack(std::vector<unsigned int> track_ids,
  ...
  for (const auto& track_id : track_ids) {
    for (const auto& h : gem_hits) {
      ...
      if (discardEleHitsGEM_ && pdgid == 11)
        continue;
  ...
  for (const auto& d : detids) {
    ...
    for (const auto& h : hits1) {
      ...
      if (verboseGEM_)
        std::cout << "GEMHits detid1 " << id1 << " pad1 " << static_cast<int>(roll1->padTopology().channel(lp))
                  << std::endl;
    }
    ...
    for (const auto& d2 : detids) {
      ...
      for (const auto& h : hits2) {
        ...
        if (verboseGEM_)
          std::cout << "GEMHits detid2 " << id2 << " pad2 " << static_cast<int>(roll2->padTopology().channel(lp))
                    << std::endl;
      }
    }
...
void SimHitMatcher::matchME0SimHitsToSimTrack(std::vector<unsigned int> track_ids,
  ...
  for (const auto& track_id : track_ids) {
    for (const auto& h : me0_hits) {
      if (h.trackId() != track_id)
        continue;
      if (verboseME0_)
        cout << "simhit in ME0 detid " << ME0DetId(h.detUnitId()) << " id " << h.particleType() << " position x "
             << h.entryPoint().x() << " y " << h.entryPoint().y() << endl;
      int pdgid = h.particleType();
      //if (simMuOnlyME0_ && std::abs(pdgid) != 13) continue;
      // discard electron hits in the ME0 chambers
      if (discardEleHitsME0_ && std::abs(pdgid) == 11)
        continue;
...
void SimHitMatcher::matchDTSimHitsToSimTrack(std::vector<unsigned int> track_ids,
...
const edm::PSimHitContainer& SimHitMatcher::simHits(enum MuonType sub) const {
  switch (sub) {
...
std::set<unsigned int> SimHitMatcher::detIdsGEM(int gem_type) const {
  std::set<unsigned int> result;
  for (const auto& p : gem_detid_to_hits_) {
    const auto& id = p.first;
    if (gem_type > 0) {
      GEMDetId detId(id);
      if (gemvalidation::toGEMType(detId.station(), detId.ring()) != gem_type)
        continue;
    }
    result.insert(id);
  }
...
std::set<unsigned int> SimHitMatcher::detIdsRPC(int rpc_type) const {
  std::set<unsigned int> result;
  for (const auto& p : rpc_detid_to_hits_) {
    const auto& id = p.first;
    if (rpc_type > 0) {
      RPCDetId detId(id);
      if (gemvalidation::toRPCType(detId.region(), detId.station(), detId.ring()) != rpc_type)
        continue;
    }
    result.insert(id);
  }
...
std::set<unsigned int> SimHitMatcher::detIdsME0() const {
  std::set<unsigned int> result;
  for (const auto& p : me0_detid_to_hits_)
...
std::set<unsigned int> SimHitMatcher::detIdsDT(int dt_type) const {
  std::set<unsigned int> result;
  for (const auto& p : dt_detid_to_hits_) {
    const auto& id = p.first;
    if (dt_type > 0) {
      DTWireId detId(id);
      if (gemvalidation::toDTType(detId.wheel(), detId.station()) != dt_type)
        continue;
    }
    result.insert(id);
  }
...
std::set<unsigned int> SimHitMatcher::detIdsME0Coincidences(int min_n_layers) const {
  std::set<unsigned int> result;
...
std::set<unsigned int> SimHitMatcher::chamberIdsGEM(int gem_type) const {
  std::set<unsigned int> result;
...
std::set<unsigned int> SimHitMatcher::chamberIdsRPC(int rpc_type) const {
  std::set<unsigned int> result;
...
std::set<unsigned int> SimHitMatcher::chamberIdsME0() const {
  std::set<unsigned int> result;
  for (const auto& p : me0_chamber_to_hits_)
    result.insert(p.first);
  return result;
}
...
std::set<unsigned int> SimHitMatcher::chamberIdsDT(int dt_type) const {
  std::set<unsigned int> result;
...
std::set<unsigned int> SimHitMatcher::superChamberIdsGEM() const {
  std::set<unsigned int> result;
  for (const auto& p : gem_superchamber_to_hits_)
    result.insert(p.first);
  return result;
}

std::set<unsigned int> SimHitMatcher::superChamberIdsME0() const {
  std::set<unsigned int> result;
  for (const auto& p : me0_superchamber_to_hits_)
    result.insert(p.first);
  return result;
}
...
std::set<unsigned int> SimHitMatcher::layerIdsDT() const {
  std::set<unsigned int> result;
  for (const auto& p : dt_layer_to_hits_)
    result.insert(p.first);
  return result;
}

std::set<unsigned int> SimHitMatcher::superlayerIdsDT() const {
  std::set<unsigned int> result;
  for (const auto& p : dt_superlayer_to_hits_)
    result.insert(p.first);
  return result;
}
...
const edm::PSimHitContainer& SimHitMatcher::hitsInDetId(unsigned int detid) const {
  ...
  if (gemvalidation::is_me0(detid)) {
    if (me0_detid_to_hits_.find(detid) == me0_detid_to_hits_.end())
      return no_hits_;
    return me0_detid_to_hits_.at(detid);
  }
  if (gemvalidation::is_csc(detid)) {
    if (csc_detid_to_hits_.find(detid) == csc_detid_to_hits_.end())
      return no_hits_;
    return csc_detid_to_hits_.at(detid);
  }
  if (gemvalidation::is_rpc(detid)) {
    if (rpc_detid_to_hits_.find(detid) == rpc_detid_to_hits_.end())
      return no_hits_;
...
const edm::PSimHitContainer& SimHitMatcher::hitsInChamber(unsigned int detid) const {
  ...
  if (gemvalidation::is_me0(detid))  // make sure we use chamber id
  {
    ...
    if (me0_chamber_to_hits_.find(id.layerId().rawId()) == me0_chamber_to_hits_.end())
      return no_hits_;
    return me0_chamber_to_hits_.at(id.layerId().rawId());
  }
  if (gemvalidation::is_csc(detid)) {
    const CSCDetId id(detid);
    if (csc_chamber_to_hits_.find(id.chamberId().rawId()) == csc_chamber_to_hits_.end())
      return no_hits_;
    return csc_chamber_to_hits_.at(id.chamberId().rawId());
  }
  if (gemvalidation::is_rpc(detid)) {
    const RPCDetId id(detid);
    if (rpc_chamber_to_hits_.find(id.chamberId().rawId()) == rpc_chamber_to_hits_.end())
      return no_hits_;
...
const edm::PSimHitContainer& SimHitMatcher::hitsInSuperChamber(unsigned int detid) const {
  ...
  if (gemvalidation::is_csc(detid))
    return hitsInChamber(detid);
  if (gemvalidation::is_me0(detid)) {
    const ME0DetId id(detid);
    if (me0_superchamber_to_hits_.find(id.chamberId().rawId()) == me0_superchamber_to_hits_.end())
      return no_hits_;
...
const edm::PSimHitContainer& SimHitMatcher::hitsInLayerDT(unsigned int detid) const {
  if (!gemvalidation::is_dt(detid))
    return no_hits_;

  const DTWireId id(detid);
  if (dt_layer_to_hits_.find(id.layerId().rawId()) == dt_layer_to_hits_.end())
    return no_hits_;
  return dt_layer_to_hits_.at(id.layerId().rawId());
}

const edm::PSimHitContainer& SimHitMatcher::hitsInSuperLayerDT(unsigned int detid) const {
  if (!gemvalidation::is_dt(detid))
    return no_hits_;

  const DTWireId id(detid);
  if (dt_superlayer_to_hits_.find(id.superlayerId().rawId()) == dt_superlayer_to_hits_.end())
    return no_hits_;
  return dt_superlayer_to_hits_.at(id.superlayerId().rawId());
}

const edm::PSimHitContainer& SimHitMatcher::hitsInChamberDT(unsigned int detid) const {
  if (!gemvalidation::is_dt(detid))
    return no_hits_;
...
bool SimHitMatcher::hitStationGEM(int st, int nlayers) const {
  int nst = 0;
...
bool SimHitMatcher::hitStationRPC(int st) const {
  int nst = 0;
...
bool SimHitMatcher::hitStationCSC(int st, int nlayers) const {
  int nst = 0;
...
bool SimHitMatcher::hitStationDT(int st, int nsuperlayers, int nlayers) const {
  int nst = 0;
...
int SimHitMatcher::nStationsCSC(int nlayers) const {
  return (hitStationCSC(1, nlayers) + hitStationCSC(2, nlayers) + hitStationCSC(3, nlayers) +
          hitStationCSC(4, nlayers));
}

int SimHitMatcher::nStationsDT(int nsuperlayers, int nlayers) const {
...
int SimHitMatcher::nStationsRPC() const {
  return (hitStationRPC(1) + hitStationRPC(2) + hitStationRPC(3) + hitStationRPC(4));
}

int SimHitMatcher::nStationsGEM(int nlayers) const { return (hitStationGEM(1, nlayers) + hitStationGEM(3, nlayers)); }

int SimHitMatcher::nCellsWithHitsInLayerDT(unsigned int detid) const {
  set<int> layers_with_hits;
...
int SimHitMatcher::nLayersWithHitsInSuperLayerDT(unsigned int detid) const {
  set<int> layers_with_hits;
...
int SimHitMatcher::nSuperLayersWithHitsInChamberDT(unsigned int detid) const {
...
int SimHitMatcher::nLayersWithHitsInChamberDT(unsigned int detid) const {
  int nLayers = 0;
...
GlobalPoint SimHitMatcher::simHitPositionKeyLayer(unsigned int chid) const {
  /*
  ...
  */
  ...
  const CSCDetId keyLayerId(chamberId.endcap(), chamberId.station(), chamberId.ring(), chamberId.chamber(), 3);
  ...
  if (keyLayerIdHits.size() != 0)
    returnValue = simHitsMeanPosition(keyLayerIdHits);
  else {
    // check if the chamber has hits at all
    if (hitsInChamber(chid).size() == 0)
      returnValue = GlobalPoint();
    //else if (hitsInChamber(chid).size()>=1) returnValue = simHitsMeanPosition(hitsInChamber(chid));
    else if (hitsInChamber(chid).size() == 1)
      returnValue = simHitsMeanPosition(hitsInChamber(chid));
    else if (hitsInChamber(chid).size() == 2)
      returnValue = simHitsMeanPosition(hitsInChamber(chid));
    else {
      std::vector<float> zs;
      std::vector<float> xs;
      ...
      for (int l = 1; l <= 6; l++) {
        CSCDetId l_id(chamberId.endcap(), chamberId.station(), chamberId.ring(), chamberId.chamber(), l);
      ...
      PtassignmentHelper::calculateAlphaBeta(zs, xs, ezs, exs, status, alphax, betax);
      float alphay = -99., betay = 0.;
      PtassignmentHelper::calculateAlphaBeta(zs, ys, ezs, exs, status, alphay, betay);
...
GlobalVector SimHitMatcher::simHitsMeanMomentum(const edm::PSimHitContainer& sim_hits) const {
  if (sim_hits.empty())
...
float SimHitMatcher::simHitsGEMCentralPosition(const edm::PSimHitContainer& sim_hits) const {
  if (sim_hits.empty())
  ...
  for (const auto& h : sim_hits) {
    ...
    if (n >= 1)
      std::cout << "warning! find more than one simhits in GEM chamber " << std::endl;
    ++n;
  }
...
float SimHitMatcher::LocalBendingInChamber(unsigned int detid) const {
  const CSCDetId cscid(detid);
  if (nLayersWithHitsInSuperChamber(detid) < 6)
    return -100;
  float phi_layer1 = -10;
  float phi_layer6 = 10;
  ...
  if (cscid.station() == 1 and (cscid.ring() == 1 or cscid.ring() == 4)) {
    ...
    else if (hits1a.size() > 0)
      phi_layer1 = gp1a.phi();
    else if (hits1b.size() > 0)
      phi_layer1 = gp1b.phi();
    else
      std::cerr << " no hits in layer1, cant not find global phi of hits " << std::endl;
    ...
    else if (hits6a.size() > 0)
      phi_layer6 = gp6a.phi();
    else if (hits6b.size() > 0)
      phi_layer6 = gp6b.phi();
    else
      std::cerr << " no hits in layer6, cant not find global phi of hits " << std::endl;
...
float SimHitMatcher::simHitsMeanWG(const edm::PSimHitContainer& sim_hits) const {
  if (sim_hits.empty())
    return -1.f;
...
float SimHitMatcher::simHitsMeanWire(const edm::PSimHitContainer& sim_hits) const {
  if (sim_hits.empty())
    return -1.f;
...
std::set<int> SimHitMatcher::hitME0PadsInDetId(unsigned int detid) const {
  set<int> none;
  if (me0_detids_to_pads_.find(detid) == me0_detids_to_pads_.end())
    return none;
...
std::set<unsigned int> SimHitMatcher::hitWiresInDTLayerId(unsigned int detid, int margin_n_wires) const {
...
std::set<unsigned int> SimHitMatcher::hitWiresInDTSuperLayerId(unsigned int detid, int margin_n_wires) const {
  set<unsigned int> result;
  const auto& layers(getDTGeometry()->superLayer(DTSuperLayerId(detid))->layers());
  for (const auto& l : layers) {
    if (verboseDT_)
      cout << "hitWiresInDTSuperLayerId::l id " << l->id() << endl;
...
std::set<unsigned int> SimHitMatcher::hitWiresInDTChamberId(unsigned int detid, int margin_n_wires) const {
  set<unsigned int> result;
  const auto& superLayers(getDTGeometry()->chamber(DTChamberId(detid))->superLayers());
  for (const auto& sl : superLayers) {
    if (verboseDT_)
      cout << "hitWiresInDTChamberId::sl id " << sl->id() << endl;
...
int SimHitMatcher::nCoincidenceCSCChambers(int min_n_layers) const {
  int result = 0;
  const auto& chamber_ids = chamberIdsCSC();
  for (const auto& id : chamber_ids) {
    if (nLayersWithHitsInSuperChamber(id) >= min_n_layers)
      result += 1;
  }
  return result;
}

void SimHitMatcher::cscChamberIdsToString(const std::set<unsigned int>& set) const {
...
void SimHitMatcher::dtChamberIdsToString(const std::set<unsigned int>& set) const {
  for (const auto& p : set) {
    DTChamberId detId(p);
    std::cout << " " << detId << "\n";
  }
...
std::set<unsigned int> SimHitMatcher::chamberIdsCSCStation(int station) const {
  set<unsigned int> result;
  switch (station) {
    case 1: {
...
std::set<unsigned int> SimHitMatcher::chamberIdsDTStation(int station) const {
  set<unsigned int> result;
  switch (station) {
    case 1: {
...
GlobalPoint SimHitMatcher::simHitsMeanPositionStation(int station) const {
