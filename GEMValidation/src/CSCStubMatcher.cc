...
CSCStubMatcher::CSCStubMatcher(const SimHitMatcher& sh,
    ...
    : DigiMatcher(sh), digi_matcher_(&dg), gem_digi_matcher_(&gem_dg), sh_matcher_(&sh) {
  ...
  if (hasCSCGeometry_) {
    ...
    if (gemvalidation::getByToken(clctInputs_, clcts, event()))
      if (runCLCT_)
        matchCLCTsToSimTrack(*clcts.product());

    edm::Handle<CSCALCTDigiCollection> alcts;
    if (gemvalidation::getByToken(alctInputs_, alcts, event()))
      if (runALCT_)
        matchALCTsToSimTrack(*alcts.product());

    edm::Handle<CSCCorrelatedLCTDigiCollection> lcts;
    if (gemvalidation::getByToken(lctInputs_, lcts, event()))
      if (runLCT_)
        matchLCTsToSimTrack(*lcts.product());

    edm::Handle<CSCCorrelatedLCTDigiCollection> mplcts;
    if (gemvalidation::getByToken(mplctInputs_, mplcts, event()))
      if (runMPLCT_)
...
void CSCStubMatcher::matchCLCTsToSimTrack(const CSCCLCTDigiCollection& clcts) {
  ...
  for (const auto& id : cathode_ids) {
    ...
    if (digi_matcher_->nLayersWithStripInChamber(id) >= minNHitsChamberCLCT_)
      ++n_minLayers;
  ...
  if (verbose() and n_minLayers > 0) {
    if (chamber_to_clct_.size() == 0) {
      ...
      for (const auto& it : clcts) {
        ...
        if (useCSCChamberType(id.iChamberType()))
          continue;
...
void CSCStubMatcher::matchALCTsToSimTrack(const CSCALCTDigiCollection& alcts) {
  ...
  for (const auto& id : anode_ids) {
    if (digi_matcher_->nLayersWithWireInChamber(id) >= minNHitsChamberALCT_)
      ++n_minLayers;
    CSCDetId ch_id(id);
...
void CSCStubMatcher::matchLCTsToSimTrack(const CSCCorrelatedLCTDigiCollection& lcts) {
  ...
  for (const auto& id : cathode_and_anode_ids) {
    if (digi_matcher_->nLayersWithStripInChamber(id) >= minNHitsChamberCLCT_ and
        digi_matcher_->nLayersWithWireInChamber(id) >= minNHitsChamberALCT_)
      ++n_minLayers;
    CSCDetId ch_id(id);
    ...
    for (auto lct = lcts_in_det.first; lct != lcts_in_det.second; ++lct) {
      ...
      if (bx_to_lcts[bx].size() == 2 and addGhostLCTs_) {
        ...
        if (!(wg1 == wg2 || hs1 == hs2)) {
          ...
          CSCCorrelatedLCTDigi LCT12(
              0, 1, digi_quality(lct12), digi_wg(lct12), digi_channel(lct12), digi_pattern(lct12), 0, digi_bx(lct12));
          cscLcts_tmp.push_back(LCT12);

          auto lct21 = lct22;
          digi_wg(lct21) = wg1;
          lcts_tmp.push_back(lct21);
          CSCCorrelatedLCTDigi LCT21(
              0, 1, digi_quality(lct21), digi_wg(lct21), digi_channel(lct21), digi_pattern(lct21), 0, digi_bx(lct21));
    ...
    if (verbose())
      cout << "number of lcts = " << n_lct << " from cscLcts_tmp " << cscLcts_tmp.size() << endl;
    if (n_lct == 0)
      continue;  // no LCTs in this chamber
    ...
    for (const auto& lct : cscLcts_tmp) {
      ...
      for (const auto& p : cscClctsInChamber(id)) {
        if (verboseLCT_)
          std::cout << "\tLCT checking match with CLCT " << p << std::endl;
        if (p == lct.getCLCT()) {
          lct_clct_match = true;
          if (verboseLCT_)
            cout << "\tLCT matched to CLCT " << p << endl;
          break;
        }
      }
      if (not lct_clct_match)
        if (verboseLCT_)
          cout << "\tLCT Failed to match to CLCT " << endl;

      // Check if matched to an ALCT
      for (const auto& p : cscAlctsInChamber(id)) {
        //ALCT BX is shifted
        if (verboseLCT_)
          std::cout << "\tLCT checking match with ALCT " << p << std::endl;
        if (p.getKeyWG() == lct.getALCT().getKeyWG()) {
          lct_alct_match = true;
          if (verboseLCT_)
            cout << "\tLCT matched to ALCT " << p << endl;
          break;
        }
      }
      if (not lct_alct_match)
        if (verboseLCT_)
          cout << "\tLCT Failed to match to ALCT " << endl;
      ...
      if (ch_id.ring() == 1 and (ch_id.station() == 1 or ch_id.station() == 2)) {
        ...
        if (lct.getType() == CSCCorrelatedLCTDigi::ALCTCLCT2GEM) {
          ...
          for (const auto& p : gem_digi_matcher_->gemPadsInChamber(gemDetIdL1.rawId())) {
            if (verboseLCT_)
              std::cout << "\tLCT checking match with GEML1 " << p << std::endl;
            if (p == lct.getGEM1()) {
              lct_gem1_match = true;
              if (verboseLCT_)
                cout << "\tLCT matched to GEML1 " << p << endl;
              break;
            }
          }
          if (not lct_gem1_match)
            if (verboseLCT_)
              cout << "\tLCT Failed to match to GEM1 " << endl;

          // Check if matched to an GEM pad L2
          for (const auto& p : gem_digi_matcher_->gemPadsInChamber(gemDetIdL2.rawId())) {
            if (verboseLCT_)
              std::cout << "\tLCT checking match with GEML2 " << p << std::endl;
            if (p == lct.getGEM2()) {
              lct_gem2_match = true;
              if (verboseLCT_)
                cout << "\tLCT matched to GEML2 " << p << endl;
              break;
            }
          }
          if (not lct_gem2_match)
            if (verboseLCT_)
              cout << "\tLCT Failed to match to GEM2 " << endl;
        }
      ...
      lct_matched = ((lct_clct_match and lct_alct_match) or (lct_alct_match and (lct_gem1_match or lct_gem2_match)) or
                     (lct_clct_match and (lct_gem1_match or lct_gem2_match)));

      if (chamber_to_lct_.find(id) == chamber_to_lct_.end())
        chamber_to_lct_[id] = lcts_tmp[iLct];
      else {
        if (verbose())
          cout << "ALARM!!! here already was matching LCT " << chamber_to_lct_[id] << " New LCT  " << lcts_tmp[iLct]
               << endl;
      }
  ...
  if (verbose() and n_minLayers > 0) {
    if (chamber_to_lct_.size() == 0) {
      ...
      cout << "at least one matched LCT" << std::endl;
  }
}
...
void CSCStubMatcher::matchMPLCTsToSimTrack(const CSCCorrelatedLCTDigiCollection& mplcts) {
  ...
  for (const auto& id : cathode_and_anode_ids) {
    ...
    for (auto lct = mplcts_in_det.first; lct != mplcts_in_det.second; ++lct) {
      ...
      if (verbose())
        cout << "mplct in detId" << ch_id << " " << *lct << endl;
    ...
    if (verbose())
      cout << "number of mplct = " << n_lct << endl;
    if (n_lct == 0)
      continue;  // no mplcts in this chamber
    ...
    for (unsigned int i = 0; i < clct.size(); i++) {
      ...
      for (unsigned int j = 0; j < alct.size(); j++) {
        ...
        if (verbose())
          cout << "will match hs" << my_hs << " wg" << my_wg << " bx" << my_bx << " to #lct " << n_lct << endl;
        for (auto& lct : mplcts_tmp) {
          if (verbose())
            cout << " corlct " << lct;
          if (is_valid(alct[j]) and is_valid(clct[i]) and
              !(my_bx == digi_bx(lct) and my_hs == digi_channel(lct) and my_wg == digi_wg(lct))) {
            if (verbose())
              cout << "  BAD" << endl;
            continue;
          }
...
matching::Digi CSCStubMatcher::clctInChamber(unsigned int detid) const {
  if (chamber_to_clct_.find(detid) == chamber_to_clct_.end())
    return make_digi();
  return chamber_to_clct_.at(detid);
}

matching::Digi CSCStubMatcher::alctInChamber(unsigned int detid) const {
  if (chamber_to_alct_.find(detid) == chamber_to_alct_.end())
    return make_digi();
  return chamber_to_alct_.at(detid);
}

matching::Digi CSCStubMatcher::lctInChamber(unsigned int detid) const {
  if (chamber_to_lct_.find(detid) == chamber_to_lct_.end())
    return make_digi();
  return chamber_to_lct_.at(detid);
}

matching::Digi CSCStubMatcher::mplctInChamber(unsigned int detid) const {
  if (chamber_to_mplct_.find(detid) == chamber_to_mplct_.end())
    return make_digi();
  return chamber_to_mplct_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::allCLCTsInChamber(unsigned int detid) const {
  if (chamber_to_clcts_all_.find(detid) == chamber_to_clcts_all_.end())
    return no_digis_;
  return chamber_to_clcts_all_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::allALCTsInChamber(unsigned int detid) const {
  if (chamber_to_alcts_all_.find(detid) == chamber_to_alcts_all_.end())
    return no_digis_;
  return chamber_to_alcts_all_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::allLCTsInChamber(unsigned int detid) const {
  if (chamber_to_lcts_all_.find(detid) == chamber_to_lcts_all_.end())
    return no_digis_;
  return chamber_to_lcts_all_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::allMPLCTsInChamber(unsigned int detid) const {
  if (chamber_to_mplcts_all_.find(detid) == chamber_to_mplcts_all_.end())
    return no_digis_;
  return chamber_to_mplcts_all_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::clctsInChamber(unsigned int detid) const {
  if (chamber_to_clcts_.find(detid) == chamber_to_clcts_.end())
    return no_digis_;
  return chamber_to_clcts_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::alctsInChamber(unsigned int detid) const {
  if (chamber_to_alcts_.find(detid) == chamber_to_alcts_.end())
    return no_digis_;
  return chamber_to_alcts_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::lctsInChamber(unsigned int detid) const {
  if (chamber_to_lcts_.find(detid) == chamber_to_lcts_.end())
    return no_digis_;
  return chamber_to_lcts_.at(detid);
}

const matching::DigiContainer& CSCStubMatcher::mplctsInChamber(unsigned int detid) const {
  if (chamber_to_mplcts_.find(detid) == chamber_to_mplcts_.end())
    return no_digis_;
  return chamber_to_mplcts_.at(detid);
}

const matching::DigiContainer CSCStubMatcher::lctsInStation(int st) const {
  DigiContainer lcts;
  for (const auto& p : chamber_to_lcts_) {
    CSCDetId id(p.first);
    if (id.station() == st)
      lcts.insert(lcts.end(), (p.second).begin(), (p.second).end());
    else
      continue;
  }
  return lcts;
...
const CSCCLCTDigiContainer& CSCStubMatcher::allCscCLCTsInChamber(unsigned int detid) const {
  if (chamber_to_cscClcts_all_.find(detid) == chamber_to_cscClcts_all_.end())
    return no_csc_clcts_;
  return chamber_to_cscClcts_all_.at(detid);
}

const CSCALCTDigiContainer& CSCStubMatcher::allCscALCTsInChamber(unsigned int detid) const {
  if (chamber_to_cscAlcts_all_.find(detid) == chamber_to_cscAlcts_all_.end())
    return no_csc_alcts_;
  return chamber_to_cscAlcts_all_.at(detid);
}

const CSCCorrelatedLCTDigiContainer& CSCStubMatcher::allCscLCTsInChamber(unsigned int detid) const {
  if (chamber_to_cscLcts_all_.find(detid) == chamber_to_cscLcts_all_.end())
    return no_csc_lcts_;
  return chamber_to_cscLcts_all_.at(detid);
}

const CSCCorrelatedLCTDigiContainer& CSCStubMatcher::allCscMPLCTsInChamber(unsigned int detid) const {
  if (chamber_to_cscMplcts_all_.find(detid) == chamber_to_cscMplcts_all_.end())
    return no_csc_mplcts_;
  return chamber_to_cscMplcts_all_.at(detid);
}

const CSCCLCTDigiContainer& CSCStubMatcher::cscClctsInChamber(unsigned int detid) const {
  if (chamber_to_cscClcts_.find(detid) == chamber_to_cscClcts_.end())
    return no_csc_clcts_;
  return chamber_to_cscClcts_.at(detid);
}

const CSCALCTDigiContainer& CSCStubMatcher::cscAlctsInChamber(unsigned int detid) const {
  if (chamber_to_cscAlcts_.find(detid) == chamber_to_cscAlcts_.end())
    return no_csc_alcts_;
  return chamber_to_cscAlcts_.at(detid);
}

const CSCCorrelatedLCTDigiContainer& CSCStubMatcher::cscLctsInChamber(unsigned int detid) const {
  if (chamber_to_cscLcts_.find(detid) == chamber_to_cscLcts_.end())
    return no_csc_lcts_;
  return chamber_to_cscLcts_.at(detid);
}

const CSCCorrelatedLCTDigiContainer& CSCStubMatcher::cscMplctsInChamber(unsigned int detid) const {
  if (chamber_to_cscMplcts_.find(detid) == chamber_to_cscMplcts_.end())
    return no_csc_mplcts_;
  return chamber_to_cscMplcts_.at(detid);
}

CSCCLCTDigiContainer CSCStubMatcher::cscClcts() const {
...
CSCCLCTDigi CSCStubMatcher::bestCscClctInChamber(unsigned int detid) const {
...
CSCALCTDigi CSCStubMatcher::bestCscAlctInChamber(unsigned int detid) const {
  //sort stubs based on quality
...
CSCCorrelatedLCTDigi CSCStubMatcher::bestCscLctInChamber(unsigned int detid) const {
  //sort stubs based on quality
...
CSCComparatorDigiDetIdContainer CSCStubMatcher::matchingComparatorDigisLCT(unsigned int detid,
                                                                           const CSCCorrelatedLCTDigi& stub) const {
...
bool CSCStubMatcher::comparatorInCLCTPattern(int keyStrip, int pattern, int layer, int halfStrip) const {
  /*
...
void CSCStubMatcher::positionsOfComparatorInLCT(unsigned int detid,
                                                const CSCCorrelatedLCTDigi& stub,
                                                std::vector<GlobalPoint>& gps) const {
  ...
  for (int l = 1; l <= 6; l++) {
    ...
    if (n > 0)
      gps.push_back(GlobalPoint(sum_x / n, sum_y / n, sum_z / n));
  }
}
...
int CSCStubMatcher::nChambersWithCLCT(int min_quality) const {
  ...
  for (const auto& id : chamber_ids) {
    ...
    if (!is_valid(clct))
      continue;
    if (digi_quality(clct) >= min_quality)
      ++result;
  }
  return result;
}
...
int CSCStubMatcher::nChambersWithALCT(int min_quality) const {
  ...
  for (const auto& id : chamber_ids) {
    ...
    if (!is_valid(alct))
      continue;
    if (digi_quality(alct) >= min_quality)
      ++result;
  }
  return result;
}
...
int CSCStubMatcher::nChambersWithLCT(int min_quality) const {
  ...
  for (const auto& id : chamber_ids) {
    ...
    if (!is_valid(lct))
      continue;
    if (digi_quality(lct) >= min_quality)
      ++result;
  }
  return result;
}
...
int CSCStubMatcher::nChambersWithMPLCT(int min_quality) const {
  ...
  for (const auto& id : chamber_ids) {
    ...
    if (!is_valid(mplct))
      continue;
    if (digi_quality(mplct) >= min_quality)
      ++result;
  }
  return result;
}

bool CSCStubMatcher::checkStubInChamber(const CSCDetId& id, const CSCCorrelatedLCTDigi& lct) const {
  //    int hs = lct->getStrip() + 1; // LCT halfstrip and wiregoup numbers start from 0
  //    int wg = lct->getKeyWG() + 1;
  //  std::cout << " checks Stubs, DetId " << id << std::endl;
  //fixedME
  float dphi = -9;
  const auto& mydigi = make_digi(id.rawId(),
                                 lct.getStrip() + 1,
                                 lct.getBX(),
                                 CSC_LCT,
                                 lct.getQuality(),
...
bool CSCStubMatcher::wasStubMatchedInChamber(const CSCDetId& id, const CSCCorrelatedLCTDigi& lct) const {
...
bool CSCStubMatcher::wasStubMatched(const CSCCorrelatedLCTDigi& lct) const {
...
float CSCStubMatcher::getFractionalStrip(const CSCCorrelatedLCTDigi& lct) const {
  return 0.5 * (lct.getStrip() + 1) - 0.25;
}

float CSCStubMatcher::getAverageBendingLCT(unsigned int rawId, const CSCCorrelatedLCTDigi& lct) const {
  CSCDetId cscId = CSCDetId(rawId);
  float hsWidth(gemvalidation::cscHalfStripWidth(cscId));

  // map the pattern number to min int bending, max int bending
  int maxBendList[CSCConstants::NUM_CLCT_PATTERNS] = {0, 11, 11, -11, 9, -9, 7, -7, 5, -5, 3};
  int minBendList[CSCConstants::NUM_CLCT_PATTERNS] = {0, 0, 7, -7, 5, -5, 3, -3, 0, 0, -3};
