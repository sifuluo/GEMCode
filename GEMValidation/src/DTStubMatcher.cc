...
DTStubMatcher::DTStubMatcher(const SimHitMatcher& sh, const edm::EDGetTokenT<DTLocalTriggerCollection>& input_)
  ...
  if (hasDTGeometry_) {
    ...
    if (gemvalidation::getByToken(input_, dt_stubs, event()))
      if (run_)
        matchDTLocalTriggersToSimTrack(*dt_stubs.product());
  }
}

DTStubMatcher::~DTStubMatcher() {}

void DTStubMatcher::matchDTLocalTriggersToSimTrack(const DTLocalTriggerCollection& stubs) {
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto d = stubs_in_det.first; d != stubs_in_det.second; ++d) {
      if (verbose_)
        cout << "dt local trigger digi " << c_id << " " << *d << endl;
      // check that the digi is within BX range
      if (d->bx() < minBX_ || d->bx() > maxBX_)
        continue;
...
std::set<unsigned int> DTStubMatcher::selectDetIds(
    const std::map<unsigned int, DTStubMatcher::DTLocalTriggerContainer>& stubs, int dt_type) const {
...
std::set<unsigned int> DTStubMatcher::chamberIds(int dt_type) const { return selectDetIds(chamber_to_stubs_, dt_type); }

const DTStubMatcher::DTLocalTriggerContainer& DTStubMatcher::dtLocalTriggersInChamber(unsigned int detid) const {
  if (chamber_to_stubs_.find(detid) == chamber_to_stubs_.end())
    return no_dt_stubs_;
  return chamber_to_stubs_.at(detid);
}

int DTStubMatcher::nDTLocalTriggersInChamber(unsigned int detid) const {
