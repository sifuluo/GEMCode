...
HLTTrackMatcher::HLTTrackMatcher(CSCRecHitMatcher& csc,
  ...
  if (gemvalidation::getByToken(recoTrackExtraInputLabel_, recoTrackExtras, event()))
    if (runRecoTrackExtra_)
      matchRecoTrackExtraToSimTrack(*recoTrackExtras.product());

  // RecoTrack
  edm::Handle<reco::TrackCollection> recoTracks;
  if (gemvalidation::getByToken(recoTrackInputLabel_, recoTracks, event()))
    if (runRecoTrack_)
      matchRecoTrackToSimTrack(*recoTracks.product());

  // RecoChargedCandidate
  edm::Handle<reco::RecoChargedCandidateCollection> recoChargedCandidates;
  if (gemvalidation::getByToken(recoChargedCandidateInputLabel_, recoChargedCandidates, event()))
    if (runRecoChargedCandidate_)
      matchRecoChargedCandidateToSimTrack(*recoChargedCandidates.product());
}

HLTTrackMatcher::~HLTTrackMatcher() {}

void HLTTrackMatcher::clear() {}

void HLTTrackMatcher::matchRecoTrackExtraToSimTrack(const reco::TrackExtraCollection& tracks) {
  if (verboseRecoTrackExtra_)
    std::cout << "Number of RecoTrackExtras: " << tracks.size() << std::endl;
  for (const auto& track : tracks) {
    // do not anlyze tracsks with large deltaR
    if (reco::deltaR(track.innerPosition(), trk().momentum()) > 0.5)
      continue;
    if (verboseRecoTrackExtra_) {
      std::cout << "RecoTrackExtra " << std::endl
                << "\tpT_inner: " << track.innerMomentum().Rho() << ", eta_inner: " << track.innerPosition().eta()
                << ", phi_inner: " << track.innerPosition().phi() << "\tpT_outer: " << track.outerMomentum().Rho()
                << ", eta_outer: " << track.outerPosition().eta() << ", phi_outer: " << track.outerPosition().phi()
                << std::endl;
      std::cout << "\tDeltaR(SimTrack, RecoTrackExtra): " << reco::deltaR(track.innerPosition(), trk().momentum())
                << std::endl;
      std::cout << "\tDeltaPt(SimTrack, RecoTrackExtra): "
                << std::fabs(track.innerMomentum().Rho() - trk().momentum().pt()) << std::endl;
    ...
    if (matchingDTSegments + matchingCSCSegments >= 2) {
      if (verboseRecoTrackExtra_) {
        std::cout << "\tRecoTrackExtra was matched! (deltaR = " << reco::deltaR(track.innerPosition(), trk().momentum())
                  << ") " << std::endl;
      }
...
void HLTTrackMatcher::matchRecoTrackToSimTrack(const reco::TrackCollection& tracks) {
  if (verboseRecoTrack_)
    std::cout << "Number of RecoTracks: " << tracks.size() << std::endl;
  int i = 0;
  for (const auto& track : tracks) {
    const double deltaR(
        reco::deltaR(track.outerEta(), track.outerPhi(), trk().momentum().eta(), trk().momentum().phi()));
    if (verboseRecoTrack_) {
      std::cout << "RecoTrack " << i + 1 << " - pT: " << track.outerPt() << ", eta: " << track.outerEta()
                << ", phi: " << track.outerPhi() << ", deltaR: " << deltaR << std::endl;
    }
...
void HLTTrackMatcher::matchRecoChargedCandidateToSimTrack(const reco::RecoChargedCandidateCollection& candidates) {
  if (verboseRecoTrack_)
    std::cout << "Number of RecoChargedCandidates: " << candidates.size() << std::endl;
  int i = 0;
  for (const auto& candidate : candidates) {
    ...
    if (verboseRecoChargedCandidate_) {
      std::cout << "RecoCandidate " << i + 1 << " - pT: " << candidate.pt() << ", eta: " << candidate.eta()
                << ", phi: " << candidate.phi() << ", deltaR: " << deltaR << std::endl;
    }
    // get the RecoTrack
    for (const auto& otherTrack : getMatchedRecoTracks()) {
      if (areRecoTrackSame(*(candidate.track()), otherTrack)) {
        if (verboseRecoTrack_)
          std::cout << "\tRecoChargedCandidate was matched!" << std::endl;
...
template <typename T>
bool HLTTrackMatcher::areRecoTrackSame(const T& l, const T& r) const {
  return (l.outerPosition() == r.outerPosition() and l.outerMomentum() == r.outerMomentum() and
          l.outerDetId() == r.outerDetId() and l.outerDetId() == r.outerDetId() and
          l.innerPosition() == r.innerPosition() and l.innerMomentum() == r.innerMomentum() and
          l.innerOk() == r.innerOk() and l.outerDetId() == r.outerDetId());
}
