...
GEMRecHitMatcher::GEMRecHitMatcher(const SimHitMatcher& sh,
                                   const edm::EDGetTokenT<GEMRecHitCollection>& gemRecHitInput_)
    : BaseMatcher(sh.trk(), sh.vtx(), sh.conf(), sh.event(), sh.eventSetup()), simhit_matcher_(&sh) {
  const auto& gemRecHit_ = conf().getParameter<edm::ParameterSet>("gemRecHit");
  ...
  if (hasGEMGeometry_) {
    ...
    if (gemvalidation::getByToken(gemRecHitInput_, gem_rechits, event()))
      if (runGEMRecHit_)
...
void GEMRecHitMatcher::matchRecHitsToSimTrack(const GEMRecHitCollection& rechits) {
  ...
  for (const auto& id : det_ids) {
    ...
    const auto& rechits_in_det = rechits.get(GEMDetId(id));
    ...
    for (auto d = rechits_in_det.first; d != rechits_in_det.second; ++d) {
      ...
      const auto& myrechit = make_digi(id, d->firstClusterStrip(), d->BunchX(), GEM_STRIP, d->clusterSize());
...
const GEMRecHitMatcher::GEMRecHitContainer& GEMRecHitMatcher::gemRecHitsInDetId(unsigned int detid) const {
  if (detid_to_gemRecHits_.find(detid) == detid_to_gemRecHits_.end())
    return no_gemRecHits_;
  return detid_to_gemRecHits_.at(detid);
}

const GEMRecHitMatcher::GEMRecHitContainer& GEMRecHitMatcher::gemRecHitsInChamber(unsigned int detid) const {
  if (chamber_to_gemRecHits_.find(detid) == chamber_to_gemRecHits_.end())
    return no_gemRecHits_;
  return chamber_to_gemRecHits_.at(detid);
}

const GEMRecHitMatcher::GEMRecHitContainer GEMRecHitMatcher::gemRecHits() const {
  GEMRecHitContainer result;
  for (const auto& id : chamberIds()) {
    const auto& rechitsInChamber(gemRecHitsInChamber(id));
...
int GEMRecHitMatcher::nLayersWithRecHitsInSuperChamber(unsigned int detid) const {
  set<int> layers;
  /*
  const auto& recHits = recHitsInSuperChamber(detid);
...
GlobalPoint GEMRecHitMatcher::recHitMeanPosition(const RecHitContainer& rechit) const {
  ...
  for (const auto& d : rechit) {
    const GlobalPoint& gp = recHitPosition(d);
    if (gp == point_zero)
      continue;
...
bool GEMRecHitMatcher::gemRecHitInContainer(const GEMRecHit& rh, const GEMRecHitContainer& c) const {
  bool isSame = false;
