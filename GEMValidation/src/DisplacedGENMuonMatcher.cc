...
DisplacedGENMuonMatcher::DisplacedGENMuonMatcher(const SimTrack& t,
  ...
  if (getProduct and run_ and sampleType_ == DarkSUSY)
    matchDisplacedGENMuonMatcherToSimTrack(*genParticles.product());
  else if (getProduct and run_ and sampleType_ == MuonGun)
    matchDisplacedGENMuonFromMuonGunMatcherToSimTrack(*genParticles.product());
...
void DisplacedGENMuonMatcher::matchDisplacedGENMuonMatcherToSimTrack(const reco::GenParticleCollection& genParticles) {
  double eq = 0.000001;
  ...
  for (auto iGenParticle = genParticles.begin(); iGenParticle != genParticles.end(); ++iGenParticle) {
    ...
    if (fabs(iGenParticle->pdgId()) == 13 and iGenParticle->status() == 1) {
      if (verbose_)
        std::cout << "Muon " << counterGenParticle << " " << iGenParticle->status() << " " << iGenParticle->pdgId()
                  << " " << iGenParticle->vx() << " " << iGenParticle->vy() << " " << iGenParticle->vz() << std::endl;
    ...
    if ((iGenParticle->status() == 22 and iGenParticle->pdgId() == 25)  // decaying (status = 3) SM Higgs (pdgId = 25)
        || (iGenParticle->status() == 22 and
            iGenParticle->pdgId() == 35)  // decaying (status = 3) CP-even Higgs (pdgId = 35)
    ) {
      if (iGenParticle->pdgId() == 25)
        if (verbose_)
          std::cout << "SM Higgs ";
      if (iGenParticle->pdgId() == 35)
        if (verbose_)
          std::cout << "CP-even Higgs ";
      if (verbose_)
        std::cout << counterGenParticle << " " << iGenParticle->status() << " " << iGenParticle->pdgId() << " "
                  << iGenParticle->vx() << " " << iGenParticle->vy() << " " << iGenParticle->vz() << std::endl;
      genH.push_back(&(*iGenParticle));  // Store the Higgs into vector
    }
    // Check if gen particle is
    if ((iGenParticle->status() == 22 and
         iGenParticle->pdgId() == 36)  // decaying (status = 3) CP-odd Higgs (pdgId = 36)
        || (iGenParticle->status() == 22 and
            iGenParticle->pdgId() == 3000022)  // decaying (status = 3) gamma_Dark (pdgId = 3000022)
        //         || ( iGenParticle->status() == 2 and iGenParticle->pdgId() == 443   ) // decaying (status = 2) J/psi (pdgId = 443)
    ) {
      if (iGenParticle->pdgId() == 36)
        if (verbose_)
          std::cout << "SM Higgs ";
      if (iGenParticle->pdgId() == 3000022)
        if (verbose_)
          std::cout << "CP-even Higgs ";
      if (verbose_)
        std::cout << counterGenParticle << " " << iGenParticle->status() << " " << iGenParticle->pdgId() << " "
    ...
    if (iGenParticle->status() == 3 and iGenParticle->pdgId() == 21) {
      if (verbose_)
        std::cout << "Gluon " << counterGenParticle << " " << iGenParticle->status() << " " << iGenParticle->pdgId()
                  << " " << iGenParticle->vx() << " " << iGenParticle->vy() << " " << iGenParticle->vz() << std::endl;
  ...
  if (genGd.size() >= 2) {
    ...
    if (verbose_)
      std::cout << "WARNING! genGd.size() < 2" << std::endl;
  }
  if (genGlu.size() >= 2) {
    for (int i = 0; i < 2; ++i) {
    ...
    if (verbose_)
      std::cout << "WARNING! genGlu.size() < 2" << std::endl;
  }
  ...
  if (genMuonGroups.size() == 2 and genMuonGroups[0].size() == 2 and genMuonGroups[1].size() == 2) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        ...
        const double deltaR(
            reco::deltaR(genGdMu_eta[i][j], genGdMu_phi[i][j], trk().momentum().eta(), trk().momentum().phi()));
        if (deltaR < minDeltaR) {
          minDeltaR = deltaR;
          index1 = i;
          index2 = j;
  ...
  if (index1 == -99 or index2 == -99) {
    if (verbose_)
      std::cout << " failed to find matched dark boson or matched gen muon " << std::endl;
    runOK_ = false;
    return;
  }
  if (verbose_)
    std::cout << " Find matched dark boson and matched gen muon index1 " << index1 << " index2 " << index2 << std::endl;
  runOK_ = true;
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        if (i == index1 and j == index2)
          std::cout << "=======  this is gen Mu matched to sim Mu ======= " << std::endl;
  ...
  matchedGenMu_dR = reco::deltaR(
      genGdMu_eta[index1][index2], genGdMu_phi[index1][index2], trk().momentum().eta(), trk().momentum().phi());

  matchedGenMu_dxy = dxy(genGdMu_px[index1][index2],
                         genGdMu_py[index1][index2],
                         genGdMu_vx[index1][index2],
                         genGdMu_vy[index1][index2],
                         genGdMu_pt[index1][index2]);
...
void DisplacedGENMuonMatcher::matchDisplacedGENMuonFromMuonGunMatcherToSimTrack(
  ...
  for (auto iGenParticle = genParticles.begin(); iGenParticle != genParticles.end(); ++iGenParticle) {
    ...
    if (fabs(iGenParticle->pdgId()) == 13 and iGenParticle->status() == 1) {
      if (verbose_)
        std::cout << "Muon " << counterGenParticle << " " << iGenParticle->status() << " (vx, vy, vz) "
                  << iGenParticle->pdgId() << " " << iGenParticle->vx() << " " << iGenParticle->vy() << " "
                  << iGenParticle->vz() << " pt " << iGenParticle->pt() << " eta " << iGenParticle->eta() << " phi "
                  << iGenParticle->phi() << " simTrack eta " << trk().momentum().eta() << " phi "
                  << trk().momentum().phi() << std::endl;
      const double deltaR(
          reco::deltaR(iGenParticle->eta(), iGenParticle->phi(), trk().momentum().eta(), trk().momentum().phi()));
      if (deltaR < minDeltaR) {
        minDeltaR = deltaR;
    ...
    if (matchedGENMuon_) {
      runOK_ = true;
      matchedGenMu_dxy = dxy(matchedGENMuon_->px(),
                             matchedGENMuon_->py(),
                             matchedGENMuon_->vx(),
                             matchedGENMuon_->vy(),
                             matchedGENMuon_->pt());
    ...
    if (verbose_)
      std::cout << " minDR " << minDeltaR << " matchedGenMu_dR " << matchedGenMu_dR << " matchedGenMu_dxy "
                << matchedGenMu_dxy << std::endl;
  }
}

double DisplacedGENMuonMatcher::dxy(double px, double py, double vx, double vy, double pt) {
  //Source: https://cmssdt.cern.ch/SDT/lxr/source/DataFormats/TrackReco/interface/TrackBase.h#119
  return (-vx * py + vy * px) / pt;
}

double DisplacedGENMuonMatcher::phiHeavyCorr(double pt, double eta, double phi, double q) {
  //  float resEta = eta;
...
double DisplacedGENMuonMatcher::invariantMass(const reco::Candidate* p1, const reco::Candidate* p2) {
