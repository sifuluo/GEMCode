...
: DigiMatcher(sh)
{
  ...
  if (hasME0Geometry_) {
    ...
    if (gemvalidation::getByToken(me0DigiInput_, me0_digis, event())) if (runME0Digi_) matchDigisToSimTrack(*me0_digis.product());

    edm::Handle<ME0PadDigiCollection> me0_pads;
    if (gemvalidation::getByToken(me0PadDigiInput_, me0_pads, event())) if (runME0Pad_) matchPadsToSimTrack(*me0_pads.product());
  }
}
...
void
ME0DigiMatcher::matchDigisToSimTrack(const ME0DigiCollection& digis)
{
  if (verboseDigi_) cout << "Matching simtrack to ME0 digis" << endl;
  ...
  for (const auto& id: det_ids)
  {
    ...
    {
      if (verboseDigi_) cout<<"ME0Digi "<<p_id<<" "<<*d<<endl;
      // check that the digi is within BX range
      if (d->bx() < minBXME0Digi_ || d->bx() > maxBXME0Digi_) continue;
...
void
ME0DigiMatcher::matchPadsToSimTrack(const ME0PadDigiCollection& pads)
{
  const auto& det_ids = simhit_matcher_->detIdsME0();
  for (const auto& id: det_ids)
  {
...
ME0DigiMatcher::detIdsDigi() const
{
  return selectDetIds(detid_to_digis_);
}


std::set<unsigned int>
ME0DigiMatcher::detIdsPad() const
{
  return selectDetIds(detid_to_pads_);
}


std::set<unsigned int>
ME0DigiMatcher::chamberIdsDigi() const
{
  return selectDetIds(chamber_to_digis_);
}


std::set<unsigned int>
ME0DigiMatcher::chamberIdsPad() const
{
  return selectDetIds(chamber_to_pads_);
}


std::set<unsigned int>
ME0DigiMatcher::superChamberIdsDigi() const
{
  return selectDetIds(superchamber_to_digis_);
}


std::set<unsigned int>
ME0DigiMatcher::superChamberIdsPad() const
{
  return selectDetIds(superchamber_to_pads_);
}


const ME0DigiContainer&
...
const ME0PadDigiContainer&
ME0DigiMatcher::padsInDetId(unsigned int detid) const
{
  if (detid_to_pads_.find(detid) == detid_to_pads_.end()) return no_pads_;
  return detid_to_pads_.at(detid);
}


const ME0PadDigiContainer&
ME0DigiMatcher::padsInChamber(unsigned int detid) const
{
  if (chamber_to_pads_.find(detid) == chamber_to_pads_.end()) return no_pads_;
  return chamber_to_pads_.at(detid);
}


const ME0PadDigiContainer&
ME0DigiMatcher::padsInSuperChamber(unsigned int detid) const
{
  if (superchamber_to_pads_.find(detid) == superchamber_to_pads_.end()) return no_pads_;
...
int
  ME0DigiMatcher::nLayersWithPadsInSuperChamber(unsigned int detid) const {
    set<int> layers;
    ME0DetId sch_id(detid);
    for (int iLayer = 1; iLayer <= 6; iLayer++) {
      ME0DetId ch_id(sch_id.region(), iLayer, sch_id.chamber(), 0);
...
std::set<int>
ME0DigiMatcher::padNumbersInDetId(unsigned int detid) const
{
  set<int> result;
