...
DTDigiMatcher::DTDigiMatcher(const SimHitMatcher& sh, const edm::EDGetTokenT<DTDigiCollection>& dtDigiInput_)
  ...
  if (hasDTGeometry_) {
    ...
    if (gemvalidation::getByToken(dtDigiInput_, dt_digis, event()))
      if (runDTDigi_)
        matchDigisToSimTrack(*dt_digis.product());
  }
}

DTDigiMatcher::~DTDigiMatcher() {}

void DTDigiMatcher::matchDigisToSimTrack(const DTDigiCollection& digis) {
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto d = digis_in_det.first; d != digis_in_det.second; ++d) {
      if (verboseDigi_)
        cout << "dt wire digi " << l_id << " " << *d << endl;
      // check that it matches a wire that was hit by SimHits from our track
      if (hit_wires.find(d->wire()) == hit_wires.end())
        continue;
      if (verboseDigi_)
        cout << "oki" << endl;
...
std::set<unsigned int> DTDigiMatcher::selectDetIds(const std::map<unsigned int, DTDigiContainer>& digis,
                                                   int dt_type) const {
...
std::set<unsigned int> DTDigiMatcher::detIds(int dt_type) const { return selectDetIds(detid_to_digis_, dt_type); }

std::set<unsigned int> DTDigiMatcher::layerIds(int dt_type) const { return selectDetIds(layer_to_digis_, dt_type); }

std::set<unsigned int> DTDigiMatcher::superLayerIds(int dt_type) const {
  return selectDetIds(superLayer_to_digis_, dt_type);
}

std::set<unsigned int> DTDigiMatcher::chamberIds(int dt_type) const { return selectDetIds(chamber_to_digis_, dt_type); }

const DTDigiMatcher::DTDigiContainer& DTDigiMatcher::digisInDetId(unsigned int detid) const {
  if (detid_to_digis_.find(detid) == detid_to_digis_.end())
    return no_dt_digis_;
  return detid_to_digis_.at(detid);
}

const DTDigiMatcher::DTDigiContainer& DTDigiMatcher::digisInLayer(unsigned int detid) const {
  if (layer_to_digis_.find(detid) == layer_to_digis_.end())
    return no_dt_digis_;
  return layer_to_digis_.at(detid);
}

const DTDigiMatcher::DTDigiContainer& DTDigiMatcher::digisInSuperLayer(unsigned int detid) const {
  if (superLayer_to_digis_.find(detid) == superLayer_to_digis_.end())
    return no_dt_digis_;
  return superLayer_to_digis_.at(detid);
}

const DTDigiMatcher::DTDigiContainer& DTDigiMatcher::digisInChamber(unsigned int detid) const {
  if (chamber_to_digis_.find(detid) == chamber_to_digis_.end())
    return no_dt_digis_;
  return chamber_to_digis_.at(detid);
}

int DTDigiMatcher::nTubesWithDigisInLayer(unsigned int detid) const {
  set<int> tubes;
  // FIXME
  return tubes.size();
}

int DTDigiMatcher::nLayersWithDigisInSuperLayer(unsigned int detId) const {
  set<int> layers;
...
int DTDigiMatcher::nSuperLayersWithDigisInChamber(unsigned int detId) const {
  set<int> superLayers;
...
std::set<int> DTDigiMatcher::wireNumbersInDetId(unsigned int detid) const {
  set<int> result;
