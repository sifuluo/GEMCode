...
RPCRecHitMatcher::RPCRecHitMatcher(const SimHitMatcher& sh,
                                   const edm::EDGetTokenT<RPCRecHitCollection>& rpcRecHitInput_)
  ...
  if (hasRPCGeometry_) {
    ...
    if (gemvalidation::getByToken(rpcRecHitInput_, rpc_rechits, event()))
      if (runRPCRecHit_)
        matchRecHitsToSimTrack(*rpc_rechits.product());
  }
}

RPCRecHitMatcher::~RPCRecHitMatcher() {}

void RPCRecHitMatcher::matchRecHitsToSimTrack(const RPCRecHitCollection& rechits) {
  if (verboseRPCRecHit_)
    cout << "Matching simtrack to RPC rechits" << endl;
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto d = rechits_in_det.first; d != rechits_in_det.second; ++d) {
      if (verboseRPCRecHit_)
        cout << "RPCRecHit " << p_id << " " << *d << endl;
      // check that the rechit is within BX range
      if (d->BunchX() < minBXRPC_ || d->BunchX() > maxBXRPC_)
        continue;
...
std::set<unsigned int> RPCRecHitMatcher::detIds() const {
...
std::set<unsigned int> RPCRecHitMatcher::chamberIds() const {
  std::set<unsigned int> result;
...
const RPCRecHitMatcher::RecHitContainer& RPCRecHitMatcher::recHitsInDetId(unsigned int detid) const {
  if (detid_to_recHits_.find(detid) == detid_to_recHits_.end())
    return no_recHits_;
  return detid_to_recHits_.at(detid);
}

const RPCRecHitMatcher::RecHitContainer& RPCRecHitMatcher::recHitsInChamber(unsigned int detid) const {
  if (chamber_to_recHits_.find(detid) == chamber_to_recHits_.end())
    return no_recHits_;
  return chamber_to_recHits_.at(detid);
}

const RPCRecHitMatcher::RPCRecHitContainer& RPCRecHitMatcher::rpcRecHitsInDetId(unsigned int detid) const {
  if (detid_to_rpcRecHits_.find(detid) == detid_to_rpcRecHits_.end())
    return no_rpcRecHits_;
  return detid_to_rpcRecHits_.at(detid);
}

const RPCRecHitMatcher::RPCRecHitContainer& RPCRecHitMatcher::rpcRecHitsInChamber(unsigned int detid) const {
  if (chamber_to_rpcRecHits_.find(detid) == chamber_to_rpcRecHits_.end())
    return no_rpcRecHits_;
  return chamber_to_rpcRecHits_.at(detid);
}

const RPCRecHitMatcher::RPCRecHitContainer RPCRecHitMatcher::rpcRecHits() const {
...
std::set<int> RPCRecHitMatcher::stripNumbersInDetId(unsigned int detid) const {
  set<int> result;
  /*
...
std::set<int> RPCRecHitMatcher::partitionNumbers() const {
  std::set<int> result;
...
GlobalPoint RPCRecHitMatcher::recHitPosition(const RecHit& rechit) const {
...
GlobalPoint RPCRecHitMatcher::recHitMeanPosition(const RecHitContainer& rechit) const {
...
bool RPCRecHitMatcher::rpcRecHitInContainer(const RPCRecHit& rh, const RPCRecHitContainer& c) const {
  bool isSame = false;
...
bool RPCRecHitMatcher::isRPCRecHitMatched(const RPCRecHit& thisRh) const {
  return rpcRecHitInContainer(thisRh, rpcRecHits());
}

bool RPCRecHitMatcher::areRPCRecHitSame(const RPCRecHit& l, const RPCRecHit& r) const {
