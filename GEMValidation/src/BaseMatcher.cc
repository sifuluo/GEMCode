...
BaseMatcher::BaseMatcher(
    const SimTrack& t, const SimVertex& v, const edm::ParameterSet& ps, const edm::Event& ev, const edm::EventSetup& es)
  ...
  for (int i = CSC_ALL; i != GEM_ME0; i++)
    useCSCChamberTypes_[i] = false;  // add ME0 as special CSC

  for (const auto& t : csc_types) {
    if (t >= 0 && t <= GEM_ME0)
      useCSCChamberTypes_[t] = true;
  }

  // empty list means use all the chamber types
  if (csc_types.empty())
    useCSCChamberTypes_[CSC_ALL] = true;

  // list of RPC chamber type numbers to use
  std::vector<int> rpc_types = conf().getParameter<std::vector<int> >("rpcStationsToUse");
  for (int i = RPC_ALL; i != RPC_MB24n; i++)
    useRPCChamberTypes_[i] = false;
  for (const auto& t : rpc_types) {
    if (t >= 0 && t <= RPC_MB24n)
      useRPCChamberTypes_[t] = true;
  }
  // empty list means use all the chamber types
  if (rpc_types.empty())
    useRPCChamberTypes_[RPC_ALL] = true;

  // list of DT chamber type numbers to use
  std::vector<int> dt_types = conf().getParameter<std::vector<int> >("dtStationsToUse");
  for (int i = DT_ALL; i != DT_MB24n; i++)
    useDTChamberTypes_[i] = false;
  for (const auto& t : dt_types) {
    if (t >= 0 && t <= DT_MB24n)
      useDTChamberTypes_[t] = true;
  }
  // empty list means use all the chamber types
  if (dt_types.empty())
    useDTChamberTypes_[DT_ALL] = true;

  // list of GEM chamber type numbers to use
  std::vector<int> gem_types = conf().getParameter<std::vector<int> >("gemStationsToUse");
  for (int i = GEM_ALL; i != GEM_ME21; i++)
    useGEMChamberTypes_[i] = false;
  for (const auto& t : gem_types) {
    if (t >= 0 && t <= GEM_ME21)
      useGEMChamberTypes_[t] = true;
  }
  // empty list means use all the chamber types
  if (gem_types.empty())
    useGEMChamberTypes_[GEM_ALL] = true;

  bool useGEM = false;
  for (int i = GEM_ALL; i != GEM_ME21; i++)
    useGEM |= useGEMChamberTypes_[i];
...
GlobalPoint BaseMatcher::propagateToZCharge(const GlobalPoint& inner_point,
                                            const GlobalVector& inner_vec,
                                            float z,
                                            int charge) const {
  ...
  if (state_start.hasError())
    std::cout << "state_start has error  " << std::endl;

  TrajectoryStateOnSurface tsos(propagator_->propagate(state_start, *my_plane));
  if (!tsos.isValid())
    std::cout << " tsos not valid " << std::endl;
  if (!tsos.isValid())
    tsos = propagatorOpposite_->propagate(state_start, *my_plane);
  ...
  if (tsos.isValid())
    return tsos.globalPosition();
  return GlobalPoint();
}

GlobalPoint BaseMatcher::propagateToZ(const GlobalPoint& inner_point, const GlobalVector& inner_vec, float z) const {
  Plane::PositionType pos(0.f, 0.f, z);
  Plane::RotationType rot;
  Plane::PlanePointer my_plane(Plane::build(pos, rot));
...
GlobalPoint BaseMatcher::propagateToZ(float z) const {
...
GlobalPoint BaseMatcher::propagateToR(const GlobalPoint& inner_point, const GlobalVector& inner_vec, float R) const {
  ...
  if (!tsos.isValid())
    tsos = propagatorOpposite_->propagate(state_start, *my_cyl);
...
GlobalPoint BaseMatcher::propagateToR(float r) const {
  GlobalPoint inner_point(vtx_.position().x(), vtx_.position().y(), vtx_.position().z());
  GlobalVector inner_vec(trk_.momentum().x(), trk_.momentum().y(), trk_.momentum().z());
  return propagateToR(inner_point, inner_vec, r);
}

GlobalPoint BaseMatcher::propagatedPositionGEM() const {
  const double eta(trk().momentum().eta());
  const int endcap((eta > 0.) ? 1 : -1);
  return propagateToZ(endcap * AVERAGE_GEM_Z);
}

GlobalPoint BaseMatcher::propagatedPositionSt2() const {
  const double eta(trk().momentum().eta());
  const int endcap((eta > 0.) ? 1 : -1);
  float AVERAGE_Z = (AVERAGE_ME21_EVEN_Z + AVERAGE_ME21_ODD_Z) / 2.0;
  if (abs(eta) < 1.2)
    return propagateToR(AVERAGE_DT2_R);
  else
    return propagateToZ(endcap * AVERAGE_Z);
}

GlobalPoint BaseMatcher::propagateFromME0ToCSC(
    const ME0Segment& segment, float pt, int charge, int st, bool evenodd) const {
...
//BaseMatcher::passDPhicut(CSCDetId id, float dPhi, float pt) const
{
  ...
  if (!(id.station() == 1 and (id.ring() == 1 or id.ring() == 4)) && !(id.station() == 2 and id.ring() == 1))
    return true;
  ...
  unsigned int LUTsize =
      (st == 1) ? sizeof(ME11GEMdPhi) / sizeof(ME11GEMdPhi[0]) : sizeof(ME21GEMdPhi) / sizeof(ME21GEMdPhi[0]);
  bool smalldphi =
      ((is_odd and fabs(dphi) < GEMdPhi[LUTsize - 2][1]) || (!is_odd and fabs(dphi) < GEMdPhi[LUTsize - 2][2]));
  if (fabs(dphi) < 99 and ((chargesign == 1 and dphi < 0) || (chargesign == 0 and dphi > 0) || smalldphi)) {
    for (unsigned int b = 0; b < LUTsize; b++) {
      ...
      if (double(pt) >= GEMdPhi[b][0]) {
        if ((is_odd && GEMdPhi[b][1] > fabs(dphi)) || (!is_odd && GEMdPhi[b][2] > fabs(dphi)))
          pass = true;
        else
          pass = false;
      }
  ...
  if (st == 2 and pt >= 15 and ((is_odd and fabs(dphi) < GEMdPhi[4][1]) || (!is_odd and fabs(dphi) < GEMdPhi[4][2])))
    pass = true;
...
const CSCLayerGeometry* BaseMatcher::retriveCSCKeyLayerGeometry(int rawid) const {
