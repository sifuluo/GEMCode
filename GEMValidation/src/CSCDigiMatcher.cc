...
CSCDigiMatcher::CSCDigiMatcher(const SimHitMatcher& sh,
  ...
  if (hasCSCGeometry_) {
    ...
    if (gemvalidation::getByToken(cscComparatorDigiInput_, comp_digis, event()))
      if (runWG_)
        matchStripsToSimTrack(*comp_digis.product());

    edm::Handle<CSCWireDigiCollection> wire_digis;
    if (gemvalidation::getByToken(cscWireDigiInput_, wire_digis, event()))
      if (runStrip_)
...
void CSCDigiMatcher::matchStripsToSimTrack(const CSCComparatorDigiCollection& comparators) {
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto c = comp_digis_in_det.first; c != comp_digis_in_det.second; ++c) {
      if (verboseStrip_)
        cout << "sdigi " << layer_id << " (strip, comparator, Tbin ) " << *c << endl;

      // check that the first BX for this digi wasn't too early or too late
      if (c->getTimeBin() < minBXCSCComp_ || c->getTimeBin() > maxBXCSCComp_)
        continue;
      ...
      if (verboseStrip_)
        cout << "Matched strip " << mydigi << endl;
...
void CSCDigiMatcher::matchWiresToSimTrack(const CSCWireDigiCollection& wires) {
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto w = wire_digis_in_det.first; w != wire_digis_in_det.second; ++w) {
      // check that the first BX for this digi wasn't too early or too late
      if (w->getTimeBin() < minBXCSCWire_ || w->getTimeBin() > maxBXCSCWire_)
        continue;
...
std::set<unsigned int> CSCDigiMatcher::selectDetIds(const Id2DigiContainer& digis, int csc_type) const {
...
const matching::DigiContainer& CSCDigiMatcher::stripDigisInDetId(unsigned int detid) const {
  if (detid_to_halfstrips_.find(detid) == detid_to_halfstrips_.end())
    return no_digis_;
  return detid_to_halfstrips_.at(detid);
}

const matching::DigiContainer& CSCDigiMatcher::stripDigisInChamber(unsigned int detid) const {
  if (chamber_to_halfstrips_.find(detid) == chamber_to_halfstrips_.end())
    return no_digis_;
  return chamber_to_halfstrips_.at(detid);
}

const matching::DigiContainer& CSCDigiMatcher::wireDigisInDetId(unsigned int detid) const {
  if (detid_to_wires_.find(detid) == detid_to_wires_.end())
    return no_digis_;
  return detid_to_wires_.at(detid);
}

const matching::DigiContainer& CSCDigiMatcher::wireDigisInChamber(unsigned int detid) const {
  if (chamber_to_wires_.find(detid) == chamber_to_wires_.end())
    return no_digis_;
  return chamber_to_wires_.at(detid);
}

const CSCComparatorDigiContainer& CSCDigiMatcher::cscComparatorDigisInDetId(unsigned int detid) const {
  if (detid_to_cschalfstrips_.find(detid) == detid_to_cschalfstrips_.end())
    return no_csc_strips_;
  return detid_to_cschalfstrips_.at(detid);
}

const CSCComparatorDigiContainer& CSCDigiMatcher::cscComparatorDigisInChamber(unsigned int detid) const {
  if (chamber_to_cschalfstrips_.find(detid) == chamber_to_cschalfstrips_.end())
    return no_csc_strips_;
  return chamber_to_cschalfstrips_.at(detid);
}

const CSCWireDigiContainer& CSCDigiMatcher::cscWireDigisInDetId(unsigned int detid) const {
  if (detid_to_cscwires_.find(detid) == detid_to_cscwires_.end())
    return no_csc_wires_;
  return detid_to_cscwires_.at(detid);
}

const CSCWireDigiContainer& CSCDigiMatcher::cscWireDigisInChamber(unsigned int detid) const {
  if (chamber_to_cscwires_.find(detid) == chamber_to_cscwires_.end())
    return no_csc_wires_;
