...
RPCDigiMatcher::RPCDigiMatcher(const SimHitMatcher& sh, const edm::EDGetTokenT<RPCDigiCollection>& rpcDigiInput_)
    : DigiMatcher(sh) {
  ...
  if (hasRPCGeometry_) {
    ...
    if (gemvalidation::getByToken(rpcDigiInput_, rpc_digis, event()))
      if (runRPCDigi_)
...
void RPCDigiMatcher::matchDigisToSimTrack(const RPCDigiCollection& digis) {
  if (verboseDigi_)
    cout << "Matching simtrack to RPC digis" << endl;
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto d = digis_in_det.first; d != digis_in_det.second; ++d) {
      if (verboseDigi_)
        cout << "RPCDigi " << p_id << " " << *d << endl;
      // check that the digi is within BX range
      if (d->bx() < minBXRPC_ || d->bx() > maxBXRPC_)
        continue;
...
std::set<unsigned int> RPCDigiMatcher::selectDetIds(const Id2DigiContainer& digis, int rpc_type) const {
...
std::set<unsigned int> RPCDigiMatcher::detIds(int rpc_type) const { return selectDetIds(detid_to_digis_, rpc_type); }

std::set<unsigned int> RPCDigiMatcher::chamberIds(int rpc_type) const {
  return selectDetIds(chamber_to_digis_, rpc_type);
}

const matching::DigiContainer& RPCDigiMatcher::digisInDetId(unsigned int detid) const {
  if (detid_to_digis_.find(detid) == detid_to_digis_.end())
    return no_digis_;
  return detid_to_digis_.at(detid);
}

const matching::DigiContainer& RPCDigiMatcher::digisInChamber(unsigned int detid) const  //use chamber raw id here
{
...
const RPCDigiMatcher::RPCDigiContainer& RPCDigiMatcher::rpcDigisInDetId(unsigned int detid) const {
  if (detid_to_rpcDigis_.find(detid) == detid_to_rpcDigis_.end())
    return no_rpc_digis_;
  return detid_to_rpcDigis_.at(detid);
}

const RPCDigiMatcher::RPCDigiContainer& RPCDigiMatcher::rpcDigisInChamber(unsigned int detid) const {
  if (chamber_to_rpcDigis_.find(detid) == chamber_to_rpcDigis_.end())
    return no_rpc_digis_;
  return chamber_to_rpcDigis_.at(detid);
}

int RPCDigiMatcher::nStrips() const {
  int n = 0;
...
std::set<int> RPCDigiMatcher::stripsInDetId(unsigned int detid) const {
  set<int> result;
...
std::set<int> RPCDigiMatcher::partitionNumbers() const {
  std::set<int> result;
...
int RPCDigiMatcher::extrapolateHsfromRPC(unsigned int id, int rpcstrip) const {
  int result = -1;
  ...
  int cscchamber =
      CSCTriggerNumbering::chamberFromTriggerLabels(rpc_id.sector(), 0, rpc_id.station(), rpc_id.subsector());
  ...
  if (rpcstrip > nStrips or rpcstrip < 0)
    return result;
