...
DisplacedMuonTriggerPtassignment::DisplacedMuonTriggerPtassignment(
    std::map<unsigned int, CSCCorrelatedLCTDigiContainer> chamberid_lcts,
  ...
  for (const auto& idlcts : chamberid_lcts_) {
    ...
    for (const auto& stub : idlcts.second) {
      if (stub.isValid())
        hasStub_st[chid.station() - 1] = true;
      else {
    ...
    if (chid.station() == 2 and hasStub_st[1] and hasStub_st[2] and
      ...
      continue;
    if (chid.chamber() % 2 == 0)
      isEven[chid.station() - 1] = true;
    if (idlcts.second.size() > 1 and verbose_ > 0)
      std::cout << "more than one LCT  available in chamber id " << chid << " LCT size " << idlcts.second.size()
                << std::endl;
    globalPositionOfLCT(idlcts.second, chid);
    //find GEMPads
    if ((chid.station() == 1 and (chid.ring() == 1 or chid.ring() == 4) and
         fabs(gp_st_layer3[0].eta()) <= me0MinEta_) or
        (chid.station() == 2 and chid.ring() == 1)) {
      ...
      if (gemvalidation::getByToken(gemPadDigiInput_, gem_pads, ev_))
        ...
        std::cout << "error failed to readout GEMPad " << std::endl;
      if (not hasGEMPad_st[chid.station() - 1])
        std::cout << "failed to find matched GEM Pad, station " << chid.station() << std::endl;
    }

    if (chid.station() == 1 and (chid.ring() == 1 or chid.ring() == 4) and
        fabs(gp_st_layer3[0].eta()) > me0MinEta_ - 0.01) {
      ...
      if (gemvalidation::getByToken(me0SegmentInput_, me0_segments, ev_))
        ...
        std::cout << "error failed to read out ME0Segment " << std::endl;
      if (not hasME0)
        if (verbose_ > 0)
          std::cout << "failed to find matched ME0 segment " << std::endl;
    }
  }
  ...
  if (hasStub_st[0] and hasStub_st[1] and hasStub_st[2]) {
    if (not(isEven[0]) and isEven[1] and isEven[2])
      npar = 0;
    else if (not(isEven[0]) and not(isEven[1]) and not(isEven[2]))
      npar = 1;
    else if (isEven[0] and isEven[1] and isEven[2])
      npar = 2;
    else if (isEven[0] and not(isEven[1]) and not(isEven[2]))
      npar = 3;
    else {
      if (verbose_ > 0)
...
DisplacedMuonTriggerPtassignment::DisplacedMuonTriggerPtassignment(
  ...
  if (doStubRecovery and atLeast1StubMissing) {
    ...
    for (int endcap = 1; endcap <= 2; endcap++) {
      ...
      if (zendcap * csctf_eta < 0)
        continue;
      for (int station = 1; station <= 4; station++) {
        // ignore station where a L1Mu stub is present!
        if (not stubMissingSt1 and station == 1)
          continue;
        if (not stubMissingSt2 and station == 2)
          continue;
        if (not stubMissingSt3 and station == 3)
          continue;
        if (not stubMissingSt4 and station == 4)
          continue;
        ...
        for (int ring = 1; ring <= 3; ring++) {
          ...
          for (int chamber = 1; chamber <= 36; chamber++) {
            ...
            if ((station == 2 or station == 3 or station == 4) and (ring == 1) and chamber > 18)
              continue;
            ...
            for (auto digiItr = range.first; digiItr != range.second; ++digiItr) {
              ...
              if (stubInCSCTFTracks(stub, l1Tracks))
                continue;

              // trigger sector must be the same
              if (triggerSector != ch_id.triggerSector())
                continue;
            ...
            if (station == 1 and ring == 1) {
              ...
              for (auto digiItr = range.first; digiItr != range.second; ++digiItr) {
                ...
                // trigger sector must be the same
                if (triggerSector != me1a_id.triggerSector())
                  continue;
...
DisplacedMuonTriggerPtassignment::DisplacedMuonTriggerPtassignment(GlobalPoint gp1,
                                                                   ...
                                                                   const edm::EventSetup& es,
                                                                   const edm::Event& ev)
    : ps_(ps), ev_(ev), es_(es) {  //sim level
  ...
  if (fabs(gp1.z()) > 100)
    ...
    std::cout << " gp_st1 x " << gp1.x() << " y " << gp1.y() << " z " << gp1.z() << " eta " << gp1.eta() << " phi "
              << gp1.phi() << std::endl;
  gp_st_layer3[1] = GlobalPoint(gp2);
  if (fabs(gp2.z()) > 100)
    hasStub_st[1] = true;
  else if (verbose_ > 0)
    std::cout << " gp_st2 x " << gp2.x() << " y " << gp2.y() << " z " << gp2.z() << " eta " << gp2.eta() << " phi "
              << gp2.phi() << std::endl;
  gp_st_layer3[2] = GlobalPoint(gp3);
  if (fabs(gp3.z()) > 100)
    hasStub_st[2] = true;
  else if (verbose_ > 0)
    std::cout << " gp_st3 x " << gp3.x() << " y " << gp3.y() << " z " << gp3.z() << " eta " << gp3.eta() << " phi "
              << gp3.phi() << std::endl;
  ...
  if (fabs(gp_ge[0].z()) > 100)
    hasGEMPad_st[0] = true;
  else if (verbose_ > 0)
    std::cout << " gp_ge11 x " << gp_ge[0].x() << " y " << gp_ge[0].y() << " z " << gp_ge[0].z() << " eta "
              << gp_ge[0].eta() << " phi " << gp_ge[0].phi() << std::endl;
  gp_ge[1] = GlobalPoint(gp6);
  if (fabs(gp_ge[1].z()) > 100)
    hasGEMPad_st[1] = true;
  else if (verbose_ > 0)
    std::cout << " gp_ge21 x " << gp_ge[1].x() << " y " << gp_ge[1].y() << " z " << gp_ge[1].z() << " eta "
              << gp_ge[1].eta() << " phi " << gp_ge[1].phi() << std::endl;
...
void DisplacedMuonTriggerPtassignment::fitComparatorsLCT(const CSCComparatorDigiCollection& hCSCComparators,
  ...
  if (phis.size() <= 2 or fabs(alpha) >= 99) {
    if (verbose_ > 0)
      std::cout << "warning, falied to fit comparator digis,num of digis: " << phis.size() << " alpha " << alpha
                << " beta " << beta << std::endl;
    alpha = csc_gp.phi();
    beta = 0.0;
  }
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (verbose_ > 0)
      std::cout << "i " << i << " fit_z " << fit_z_layers[i] << " fit_phi " << fit_phi_layers[i] << " perp " << perp
                << std::endl;
  }
}
...
void DisplacedMuonTriggerPtassignment::fitTrackRadius(GlobalPoint* gps, float* radius) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (fabs(radius[i] - gps[i].perp()) >= .02 * gps[i].perp() and hasStub_st[i]) {
      ...
      //if (verbose_>=0)
      if (verbose_ > 0)
        std::cout << "station " << i + 1 << " z " << gps[i].z() << " radius from gp " << gps[i].perp() << " from fit "
                  << radius[i] << std::endl;
    }
  }
}
...
void DisplacedMuonTriggerPtassignment::globalPositionOfLCT(const CSCCorrelatedLCTDigi stub, CSCDetId chid) {
  ...
  int st = chid.station();
  if (verbose_)
    std::cout << "To find globalPositionOfLCT detid " << chid << " stub " << stub << std::endl;
  if (not stub.isValid())
    return;
  ...
  if (verbose_ > 0)
    std::cout << "LCT position, chid " << chid << " hs " << stub.getStrip() + 1 << " wg " << stub.getKeyWG() + 1
              << " gp eta " << gp_st_layer3[st - 1].eta() << " phi " << gp_st_layer3[st - 1].phi() << " perp "
              << gp_st_layer3[st - 1].perp() << std::endl;
}
...
void DisplacedMuonTriggerPtassignment::globalPositionOfLCT(CSCCorrelatedLCTDigiContainer stubs, CSCDetId chid) {
  ...
  if (beststub >= stubs.size() or dR >= 99)
    if (verbose_ > 0)
      std::cout << "error beststub >= stubs.size() , beststub " << beststub << " stubs size " << stubs.size() << " dR "
                << dR << std::endl;

  globalPositionOfLCT(stubs[beststub], chid);
  if (verbose_ > 0)
    std::cout << "LCT position, chid " << chid << " hs " << stubs[beststub].getStrip() + 1 << " wg "
              << stubs[beststub].getKeyWG() + 1 << " gp eta " << gp_st_layer3[st - 1].eta() << " phi "
              << gp_st_layer3[st - 1].phi() << " perp " << gp_st_layer3[st - 1].perp() << std::endl;
}
...
void DisplacedMuonTriggerPtassignment::globalPositionOfGEMPad(const GEMPadDigi gempad, GEMDetId gemid) {
  ...
  if (gemid.station() == 1) {
    ...
    phi_gem[0] = gp_ge[0].phi();
    if (verbose_ > 0)
      std::cout << " gempad in GE11 id " << gemid << " gp eta " << gp_ge[0].eta() << " phi " << gp_ge[0].phi()
                << " pad " << gempad.pad() << std::endl;
    ...
    phi_gem[1] = gp_ge[1].phi();
    if (verbose_ > 0)
      std::cout << " gempad in GE21 id " << gemid << " gp eta " << gp_ge[1].eta() << " phi " << gp_ge[1].phi()
                << " pad " << gempad.pad() << std::endl;
  } else if (verbose_ > 0)
...
void DisplacedMuonTriggerPtassignment::globalPositionOfGEMPad(GEMPadDigiContainer gempads, GEMDetId gemid) {
  ...
  for (const auto& gempad : gempads) {
    ...
    if (hasStub_st[st - 1] and fabs(dphi) < minGEMCSCdPhi_ and
        fabs(gp_pad.eta() - gp_st_layer3[st - 1].eta()) < minGEMCSCdEta_ and
        fabs(dphi) < fabs(dphi_gemcsc_st[st - 1])) {
      gp_ge[st - 1] = GlobalPoint(gp_pad);
      dphi_gemcsc_st[st - 1] = fabs(deltaPhi(float(gp_pad.phi()), float(gp_st_layer3[st - 1].phi())));
      if (verbose_ > 0)
        std::cout << "GEMid " << gemid << " found matched GEMPad phi " << phi_gem[st - 1] << " dphi "
                  << dphi_gemcsc_st[st - 1] << std::endl;
    }
  }
}
...
void DisplacedMuonTriggerPtassignment::matchGEMPadsToTrack(const GEMPadDigiCollection& gemPads, GEMDetId id) {
  ...
  for (auto iC = gemPads.begin(); iC != gemPads.end(); iC++) {
    ...
    if (p_id.station() != id.station() or p_id.chamber() != id.chamber() or p_id.region() != id.region())
      continue;
    ...
    for (auto d = pads.first; d != pads.second; d++) {
      ...
      if (hasStub_st[st - 1] and fabs(dphi) < minGEMCSCdPhi_ and
        ...
        if (verbose_ > 0)
          std::cout << "GEMid " << p_id << " found matched GEMPad " << d->pad() << " phi " << phi_gem[st - 1]
                    << " dphi " << dphi_gemcsc_st[st - 1] << std::endl;
      }
    }
  }
}
...
void DisplacedMuonTriggerPtassignment::matchME0SegmentsToTrack(const ME0SegmentCollection& me0segments) {
  for (auto iC = me0segments.id_begin(); iC != me0segments.id_end(); ++iC) {
    ...
    for (auto iS = ch_segs.first; iS != ch_segs.second; ++iS) {
      ...
      if (verbose_)
        std::cout << "CSC stub gp eta " << gp_st_layer3[0].eta() << " phi " << gp_st_layer3[0].phi() << " gpME0 eta "
                  << gpME0.eta() << " phi " << gpME0.phi() << std::endl;
      ...
      if (fabs(dPhi) < minGEMCSCdPhi_ and fabs(dEta) < minGEMCSCdEta_ and fabs(dPhi) < fabs(dphi_me0_st1)) {
        //if (verbose_)
        std::cout << "Found matched ME0, id " << iS->me0DetId() << " phi " << gpME0.phi() << " eta " << gpME0.eta()
                  << std::endl;
        phi_me0 = gpME0.phi();
        gp_me0 = gpME0;
        dphi_me0_st1 = dPhi;
...
float DisplacedMuonTriggerPtassignment::deltadeltaYcalculation(
    GlobalPoint gp1, GlobalPoint gp2, GlobalPoint gp3, float eta, int par) const {
  float anglea = gp2.phi();
  float newyst1 = -gp1.x() * sin(anglea) + gp1.y() * cos(anglea);
  float newyst2 = -gp2.x() * sin(anglea) + gp2.y() * cos(anglea);
  ...
  if (meRing_st1 == 1 and neta == 1)
    neta = 2;
  if (meRing_st1 == 2 and neta == 2)
    neta = 1;

  if (par < 0 or par > 3 or neta == -1)
    return -99;
...
float DisplacedMuonTriggerPtassignment::phiMomentum_Xfactor(float phi_CSC, float phi_GEM, float xfactor) const {
  ...
  float phi_diff = 0.0;
  if (fabs(y) > 0.0)
    phi_diff = atan(sin(dphi) / y);
  else
    phi_diff = M_PI / 2.0;

  if (phi_diff <= -M_PI)
    phi_diff = phi_diff + 2 * M_PI;
  ...
  if (npar >= 0 and npar <= 3) {
    ...
    for (int i = 0; i < PtassignmentHelper::NPt2; i++) {
      if (fabs(ddY123) <= PtassignmentHelper::PositionbasedDDYLUT[i][neta][npar])
        ...
        break;
      if (verbose_ > 0)
        std::cout << "eta " << eta_st2 << " neta " << neta << " npar " << npar << " fabs ddY123 " << fabs(ddY123)
                  << " cut " << PtassignmentHelper::PositionbasedDDYLUT[i][neta][npar] << " position pt " << position_pt
                  << std::endl;
    }
  }
  return true;
...
bool DisplacedMuonTriggerPtassignment::runDirectionbased(bool useME0GE11, bool useGE21) {
  if (useGE21 and meRing == 1)
    return runDirectionbasedGE21(useME0GE11);
  else
    return runDirectionbasedCSConly(useME0GE11);
}
...
bool DisplacedMuonTriggerPtassignment::runDirectionbasedGE21(bool useME0GE11) {
  ...
  if (meRing == 1 and fabs(gp_st_layer3[0].eta()) <= me0MinEta_ and useME0GE11) {
    ...
    xfactor_st1 = xfactor * fabs(z_st_layers[0][0] - z_st_layers[0][5]) /
                  (xfactor * fabs(gp_st_layer3[0].z() - z_st_layers[0][5]) + 1);
  ...
  if (meRing == 1 and hasGEMPad_st[1]) {
    xfactor_st2 = xfactor * fabs(gp_ge[1].z() - gp_st_layer3[1].z()) /
                  (xfactor * fabs(gp_st_layer3[0].z() - gp_st_layer3[1].z()) + 1);
    phiM_st2 = phiMomentum_Xfactor(gp_st_layer3[1].phi(), phi_gem[1], xfactor_st2);

  } else if (meRing == 2) {
    xfactor_st2 = xfactor * fabs(z_st_layers[1][0] - z_st_layers[1][5]) /
                  (xfactor * fabs(gp_st_layer3[0].z() - z_st_layers[1][5]) + 1);
    phiM_st2 = phiMomentum_Xfactor(gp_st_layer6[1].phi(), gp_st_layer1[1].phi(), xfactor_st2);  //

  } else
    return false;

  float xfactor_st12 = xfactor * fabs(gp_st_layer3[0].z() - gp_st_layer3[1].z()) /
                       (xfactor * fabs(gp_st_layer3[0].z() - gp_st_layer3[1].z()) + 1);
  float xfactor_st23 = xfactor * fabs(gp_st_layer3[1].z() - gp_st_layer3[2].z()) /
                       (xfactor * fabs(gp_st_layer3[0].z() - gp_st_layer3[2].z()) + 1);
  ...
  if (verbose_ > 0)
    std::cout << "DisplacedMuonTrigger, direction with GE21, meRing " << meRing << " xfactor_st1 " << xfactor_st1
              << " phiM_st1 " << phiM_st1 << " xfactor_st2 " << xfactor_st2 << " phiM_st2 " << phiM_st2 << std::endl;
  ...
  if (fabs(dPhi_dir_st1_st2) > 4) {
    std::cout << "error in runDirectionbased withGE21, dPhi_dir_st1_st2 " << dPhi_dir_st1_st2 << " phiM_st1 "
              << phiM_st1 << " phiM_st2 " << phiM_st2 << std::endl;
    return false;
  }
...
bool DisplacedMuonTriggerPtassignment::runDirectionbasedCSConly(bool useME0GE11) {
  ...
  if (verbose_ > 0)
    std::cout << "DisplacedMuonTrigger CSConly direction, meRing " << meRing << " xfactor_st1 " << xfactor_st1
              << " phiM_st1 " << phiM_st1 << " xfactor_st2 " << xfactor_st2 << " phiM_st2 " << phiM_st2 << std::endl;
  ...
  if (fabs(dPhi_dir_st1_st2) > 4) {
    std::cout << "error in runDirectionbased withoutGE21, dPhi_dir_st1_st2 " << dPhi_dir_st1_st2 << " phiM_st1 "
              << phiM_st1 << " phiM_st2 " << phiM_st2 << std::endl;
    return false;
  }
...
void DisplacedMuonTriggerPtassignment::runHybrid(bool useME0GE11, bool useGE21) {
  ...
  if (npar >= 0 and npar <= 3) {
    ...
    for (int i = 0; i < PtassignmentHelper::NPt - 1; i++) {
      if (useGE21 and
          PtassignmentHelper::ellipse(
              PtassignmentHelper::HybridDDYAndDeltaPhiLUT[i][neta][npar][0],
              ...
              PtassignmentHelper::HybridDDYAndDeltaPhiLUT[i][neta][npar][4],
              ddY123,
              dPhi_dir_st1_st2) <= 1)
        hybrid_pt = PtassignmentHelper::PtBins[i];
      else if (not(useGE21) and
               PtassignmentHelper::ellipse(
                   PtassignmentHelper::HybridDDYAndDeltaPhiLUTME21CSConly[i][neta][npar][0],
                   ...
                   PtassignmentHelper::HybridDDYAndDeltaPhiLUTME21CSConly[i][neta][npar][4],
                   ddY123,
                   dPhi_dir_st1_st2) <= 1)
        hybrid_pt = PtassignmentHelper::PtBins[i];
        ...
        break;
      if (verbose_ > 0)
        std::cout << "eta_st2 " << eta_st2 << " npar " << npar << " charge " << charge << " ddY123 " << ddY123
                  << " dphi_dir " << dPhi_dir_st1_st2 << " hybrid_pt " << hybrid_pt << std::endl;
    }
  }
}
...
float DisplacedMuonTriggerPtassignment::getlocalPhiDirection(int st) const {
  ...
  if (st == 1 and hasStub_st[0])
    return phiM_st1;
  else if (st == 2 and hasStub_st[1])
    return phiM_st2;
  else if (st == 12 and hasStub_st[0] and hasStub_st[1])
    return phiM_st12;
  else if (st == 23 and hasStub_st[1] and hasStub_st[2])
    return phiM_st23;
  else {
    std::cout << " error in getlocalPhiDirection, st  " << st << " not in range or not not have stub or GEMpad"
              << std::endl;
    return -99;
  }
}
...
float DisplacedMuonTriggerPtassignment::getdeltaPhiDirection(int st1, int st2) const {
  if (((st1 == 1 and st2 == 2) or (st1 == 1 and st2 == 2)) and hasStub_st[0] and hasStub_st[1])
    return dPhi_dir_st1_st2;
  else if (((st1 == 1 and st2 == 12) or (st1 == 12 and st2 == 1)) and hasStub_st[0] and hasStub_st[1])
    return dPhi_dir_st1_st12;
  else if (((st1 == 2 and st2 == 23) or (st1 == 23 and st2 == 2)) and hasStub_st[1] and hasStub_st[2])
    return dPhi_dir_st2_st23;
  else if (((st1 == 12 and st2 == 23) or (st1 == 23 and st2 == 12)) and hasStub_st[0] and hasStub_st[1] and
           hasStub_st[2])
    return dPhi_dir_st12_st23;
  else {
    std::cout << " error in getdeltaPhiDirection, st1 " << st1 << " st2 " << st2
              << " not in range or not not have stub or GEMpad" << std::endl;
    return -99;
  }
}

int DisplacedMuonTriggerPtassignment::getHalfStrip(const CSCComparatorDigi& digi) {
  return (digi.getStrip() - 1) * 2 + digi.getComparator();
}

float DisplacedMuonTriggerPtassignment::getFractionalStrip(const CSCComparatorDigi& d) {
  return d.getStrip() + d.getComparator() / 2. - 3 / 4.;
}

bool DisplacedMuonTriggerPtassignment::stubInDTTFTracks(const L1MuDTTrackSegPhi& candidateStub,
...
bool DisplacedMuonTriggerPtassignment::stubInCSCTFTracks(const CSCCorrelatedLCTDigi& candidateStub,
...
double phiL1DTTrack(const L1MuDTTrack& track) {
  ...
  double dttrk_phi_global =
      normalizedPhi((phi_local * (M_PI / 72.)) +
                    ((M_PI / 6.) * track.spid().sector()));  // + 12*i->scNum(); //range: -16 < phi_global < 147
