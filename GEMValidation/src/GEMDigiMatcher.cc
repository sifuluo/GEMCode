...
GEMDigiMatcher::GEMDigiMatcher(const SimHitMatcher& sh,
  ...
  if (hasGEMGeometry_) {
    ...
    if (gemvalidation::getByToken(gemDigiInput_, gem_digis, event()))
      if (runGEMDigi_)
        matchDigisToSimTrack(*gem_digis.product());

    edm::Handle<GEMPadDigiCollection> gem_pads;
    if (gemvalidation::getByToken(gemPadDigiInput_, gem_pads, event()))
      if (runGEMPad_)
        matchPadsToSimTrack(*gem_pads.product());

    edm::Handle<GEMCoPadDigiCollection> gem_co_pads;
    if (gemvalidation::getByToken(gemCoPadDigiInput_, gem_co_pads, event()))
      if (runGEMCoPad_)
...
void GEMDigiMatcher::matchDigisToSimTrack(const GEMDigiCollection& digis) {
  ...
  for (const auto& id : det_ids) {
    ...
    for (auto d = digis_in_det.first; d != digis_in_det.second; ++d) {
      ...
      if (d->bx() < minBXGEMDigi_ || d->bx() > maxBXGEMDigi_)
        continue;
      ...
      if (std::find(detid_to_gempads_2strip_[id].begin(), detid_to_gempads_2strip_[id].end(), pad) ==
          detid_to_gempads_2strip_[id].end())
...
void GEMDigiMatcher::matchPadsToSimTrack(const GEMPadDigiCollection& pads) {
  if (verbosePad_)
    cout << "Matching simtrack to GEM pads" << endl;
  const auto& det_ids = simhit_matcher_->detIdsGEM();
  for (const auto& id : det_ids) {
    GEMDetId p_id(id);
    GEMDetId superch_id(p_id.region(), p_id.ring(), p_id.station(), 0, p_id.chamber(), 0);
    ...
    for (auto pad = pads_in_det.first; pad != pads_in_det.second; ++pad) {
      ...
      if (pad->bx() < minBXGEMPad_ || pad->bx() > maxBXGEMPad_)
        continue;
      if (verbosePad_)
        cout << "chp1" << endl;
  ...
  for (auto cItr = co_pads.begin(); cItr != co_pads.end(); ++cItr) {
    ...
    for (auto pad = (*cItr).second.first; pad != (*cItr).second.second; ++pad) {
      ...
      if (matchL1 and matchL2) {
        ...
        if (verboseCoPad_)
          cout << "\tWas Not Matched!" << endl;
      }
    }
  }
}

std::set<unsigned int> GEMDigiMatcher::selectDetIds(const Id2DigiContainer& digis, int gem_type) const {
...
const GEMDigiContainer& GEMDigiMatcher::gemDigisInDetId(unsigned int detid) const {
  if (detid_to_gemdigis_.find(detid) == detid_to_gemdigis_.end())
    return no_gem_digis_;
  return detid_to_gemdigis_.at(detid);
}

const GEMDigiContainer& GEMDigiMatcher::gemDigisInChamber(unsigned int detid) const {
  if (chamber_to_gemdigis_.find(detid) == chamber_to_gemdigis_.end())
    return no_gem_digis_;
  return chamber_to_gemdigis_.at(detid);
}

const GEMDigiContainer& GEMDigiMatcher::gemDigisInSuperChamber(unsigned int detid) const {
  if (superchamber_to_gemdigis_.find(detid) == superchamber_to_gemdigis_.end())
    return no_gem_digis_;
  return superchamber_to_gemdigis_.at(detid);
}

const GEMPadDigiContainer& GEMDigiMatcher::gemPadsInDetId(unsigned int detid) const {
  if (detid_to_gempads_.find(detid) == detid_to_gempads_.end())
    return no_gem_pads_;
  return detid_to_gempads_.at(detid);
}

const GEMPadDigiContainer& GEMDigiMatcher::gemPadsInChamber(unsigned int detid) const {
  if (chamber_to_gempads_.find(detid) == chamber_to_gempads_.end())
    return no_gem_pads_;
  return chamber_to_gempads_.at(detid);
}

const GEMPadDigiContainer& GEMDigiMatcher::gemPadsInSuperChamber(unsigned int detid) const {
  if (superchamber_to_gempads_.find(detid) == superchamber_to_gempads_.end())
    return no_gem_pads_;
  return superchamber_to_gempads_.at(detid);
}

const GEMCoPadDigiContainer& GEMDigiMatcher::gemCoPadsInSuperChamber(unsigned int detid) const {
  if (superchamber_to_gemcopads_.find(detid) == superchamber_to_gemcopads_.end())
...
int GEMDigiMatcher::extrapolateHsfromGEMPad(unsigned int id, int gempad) const {
  int result = -1;
  ...
  if (gempad > nGEMPads or gempad < 0)
    result = -1;
...
int GEMDigiMatcher::extrapolateHsfromGEMStrip(unsigned int id, int gemstrip) const {
  int result = -1;
  ...
  if (gemstrip > nGEMStrips or gemstrip < 0)
    result = -1;
...
std::vector<GlobalPoint> GEMDigiMatcher::positionPad1InDetId(unsigned int id) const {
  std::vector<GlobalPoint> result;
  bool verbose = false;
  GEMDetId gem_id(id);
  if (verbose)
    std::cout << "In function positionPad2InDetId gem_id " << gem_id << std::endl;
  for (const auto& p : gemDigisInDetId(id)) {
    ...
    if (std::find(result.begin(), result.end(), gem_gp) == result.end())
      result.push_back(gem_gp);
  }
  return result;
}

std::vector<GlobalPoint> GEMDigiMatcher::positionPad2InDetId(unsigned int id) const {
  bool verbose = false;
  std::vector<GlobalPoint> result;
  GEMDetId gem_id(id);
  if (verbose)
  ...
  for (const auto& p : gemDigisInDetId(id)) {
    if (verbose)
      std::cout << "Strip " << p.strip() << std::endl;
    ...
    if (verbose)
      std::cout << "middle strip " << middleStripOfPad << std::endl;
...
std::vector<GlobalPoint> GEMDigiMatcher::positionPad4InDetId(unsigned int id) const {
  bool verbose = false;
  std::vector<GlobalPoint> result;
  GEMDetId gem_id(id);
  if (verbose)
    std::cout << "In function positionPad4InDetId gem_id " << gem_id << std::endl;
  for (const auto& p : gemDigisInDetId(id)) {
    if (verbose)
      std::cout << "Strip " << p.strip() << std::endl;
    float middleStripOfPad = 0;
...
std::vector<GlobalPoint> GEMDigiMatcher::positionPad8InDetId(unsigned int id) const {
  bool verbose = false;
  std::vector<GlobalPoint> result;
  GEMDetId gem_id(id);
  if (verbose)
    std::cout << "In function positionPad8InDetId gem_id " << gem_id << std::endl;
  for (const auto& p : gemDigisInDetId(id)) {
    if (verbose)
      std::cout << "Strip " << p.strip() << std::endl;
    float middleStripOfPad = 0;
