...
CSCRecHitMatcher::CSCRecHitMatcher(const SimHitMatcher& sh,
                                   const edm::EDGetTokenT<CSCRecHit2DCollection>& cscRecHit2DInput_,
                                   const edm::EDGetTokenT<CSCSegmentCollection>& cscSegmentInput_)
    : BaseMatcher(sh.trk(), sh.vtx(), sh.conf(), sh.event(), sh.eventSetup()), simhit_matcher_(&sh) {
  const auto& cscRecHit2D = conf().getParameter<edm::ParameterSet>("cscRecHit");
  ...
  const auto& cscSegment2D = conf().getParameter<edm::ParameterSet>("cscSegment");
  ...
  if (hasCSCGeometry_) {
    ...
    if (gemvalidation::getByToken(cscRecHit2DInput_, csc_rechits, event()))
      if (runCSCRecHit2D_)
        matchCSCRecHit2DsToSimTrack(*csc_rechits.product());

    edm::Handle<CSCSegmentCollection> csc_2DSegments;
    if (gemvalidation::getByToken(cscSegmentInput_, csc_2DSegments, event()))
      if (runCSCSegment_)
        matchCSCSegmentsToSimTrack(*csc_2DSegments.product());
  }
}
...
void CSCRecHitMatcher::matchCSCRecHit2DsToSimTrack(const CSCRecHit2DCollection& rechits) {
  ...
  const auto& layer_ids = simhit_matcher_->detIdsCSC(0);
  if (verboseCSCRecHit2D_)
    cout << "Number of matched csc layer_ids " << layer_ids.size() << endl;

  for (const auto& id : layer_ids) {
    CSCDetId p_id(id);

    // print all the wires in the CSCChamber
    const auto& hit_wg(simhit_matcher_->hitWiregroupsInDetId(id));
    if (verboseCSCRecHit2D_) {
    ...
    const auto& hit_strips(simhit_matcher_->hitStripsInDetId(id));
    if (verboseCSCRecHit2D_) {
    ...
    for (auto d = rechits_in_det.first; d != rechits_in_det.second; ++d) {
      ...
      for (size_t iS = 0; iS < d->nStrips(); ++iS) {
        if (std::find(hit_strips.begin(), hit_strips.end(), d->hitWire()) != hit_wg.end())
          stripMatch = true;
      }
...
void CSCRecHitMatcher::matchCSCSegmentsToSimTrack(const CSCSegmentCollection& cscSegments) {
  ...
  // fetch all chamberIds with simhits
  const auto& chamber_ids = simhit_matcher_->chamberIdsCSC(0);
  if (verboseCSCSegment_)
    cout << "Number of matched csc segments " << chamber_ids.size() << endl;
  for (const auto& id : chamber_ids) {
    ...
    for (auto d = segments_in_det.first; d != segments_in_det.second; ++d) {
      ...
      int rechitsFound = 0;
      if (verboseCSCSegment_)
        cout << recHits.size() << " csc rechits from segment " << endl;
...
const CSCRecHitMatcher::CSCRecHit2DContainer& CSCRecHitMatcher::cscRecHit2DsInLayer(unsigned int detid) const {
...
const CSCRecHitMatcher::CSCRecHit2DContainer& CSCRecHitMatcher::cscRecHit2DsInChamber(unsigned int detid) const {
  if (chamber_to_cscRecHit2D_.find(detid) == chamber_to_cscRecHit2D_.end())
    return no_cscRecHit2Ds_;
  return chamber_to_cscRecHit2D_.at(detid);
}

const CSCRecHitMatcher::CSCSegmentContainer& CSCRecHitMatcher::cscSegmentsInChamber(unsigned int detid) const {
...
const CSCRecHitMatcher::CSCRecHit2DContainer CSCRecHitMatcher::cscRecHit2Ds() const {
...
const CSCRecHitMatcher::CSCSegmentContainer CSCRecHitMatcher::cscSegments() const {
...
int CSCRecHitMatcher::nCSCRecHits() const {
  int n = 0;
